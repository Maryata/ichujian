 ...
 * 
 */
//@Component("eKGameTaskAction")
public class EKGameTaskAction extends EKGameBaseAction {
	...
	public String reward() {
		...
		log.info("into EKGameTaskAction.reward");
		...
		try {
			...
			log.error("EKGameTaskAction.reward failed,e : " + e);
		}
	...
	public String signRecord() {
		Map<String, Object> reqMap = new HashMap<String, Object>();
		String uid = this.getParameter("uid");// 用户id
		log.info("into EKGameTaskAction.showSign");
		log.info(uid + " = uid");
		try {
			...
			reqMap.put("status", "N");
			reqMap.put("info", "1003," + e.getMessage());
			log.error("EKGameTaskAction.showSign failed,e : " + e);
		}
		out = JSONObject.fromObject(reqMap).toString();
		return "success";
	}
	...
	public String signIn() {
		...
		log.info("into EKGameTaskAction.signIn");
		...
		try {
			...
			reqMap.put("status", "N");
			reqMap.put("info", "1003, " + e.getMessage());
			log.error("EKGameTaskAction.signIn failed,e : " + e);
		}
		out = JSONObject.fromObject(reqMap).toString();
		return "success";
	}
