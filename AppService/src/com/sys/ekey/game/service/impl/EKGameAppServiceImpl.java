...
public class EKGameAppServiceImpl extends EKGameBaseService implements EKGameAppService {

    private Logger log = Logger.getLogger(EKGameAppServiceImpl.class);
    ...
    public Map<String, Object> getGameDetailByGameId(Long gid) {
        List<Map<String, Object>> list = _queryForList("ek_gameApp.getGameDetailByGameId", gid);
    ...
    public Integer userRating(Long uid, Long gid, Date date, Float grade,
        ...
        try {
            List<Map<String, Object>> userGrade = getSqlMapClientTemplate().queryForList("ek_gameApp.getGrade", _m);

            if (null == userGrade || userGrade.size() <= 0)
                // 这边返回一个Object对象，可能为null， 所以假定语句执行不异常认为是成功，返回1.
                getSqlMapClientTemplate().insert("ek_gameApp.userRating", parameterObject);
            else
                getSqlMapClientTemplate().update("ek_gameApp.updateUserRating", parameterObject);
    ...
    public Integer persistentUserBehavior(Long uid, Long gid, Date date,
        ...
        try {
            if (gid == 0) { // 通过jar名获取游戏ID
                List<Map<String, Object>> l_gid = getSqlMapClientTemplate().queryForList("ek_gameApp.getGidByJarname", jarname);
            ...
            // 这边返回一个Object对象，可能为null， 所以假定语句执行不异常认为是成功，返回1.
            getSqlMapClientTemplate().insert("ek_gameApp.persistentUserBehavior",
                    parameterObject);
    ...
    public List<Map<String, Object>> gameInfo() {
        return this.getSqlMapClientTemplate().queryForList("ek_gameApp.gameInfo");
    }
    ...
    public String getLogo(String jarName) {
        ...
        return (String) this.getSqlMapClientTemplate().queryForObject("ek_gameApp.getLogo", paramMap);
    }
    ...
    public List<Map<String, Object>> categoryList() {
        return this.getSqlMapClientTemplate().queryForList("ek_gameApp.categoryList");
    }
    ...
    public List<Map<String, Object>> gamesInCategory(String cid, String pageindex) {
        ...
        List<Map<String, Object>> list = this.getSqlMapClientTemplate().queryForList("ek_gameApp.gamesInCategory", paramMap);
        if (null != list && list.size() > 0) {
            for (Map<String, Object> map : list) {
                String id = map.get("C_ID").toString();
                /** 下载量 */
                // 根据游戏id查询真实下载量
                String realDownload = (String) this.getSqlMapClientTemplate().queryForObject("ek_gameApp.realDownload", id);
                // 如果真实下载量为空，设置值为0
                realDownload = StrUtils.isEmpty(realDownload) ? "0" : realDownload;
                map.put("C_DOWNLOAD", realDownload);
                /** 评分 */
                // 查询真实评分
                String realGrade = (String) this.getSqlMapClientTemplate().queryForObject("ek_gameApp.realGrade", id);
                // 如果真实评分为空，设置值为0
                realGrade = StrUtils.isEmpty(realGrade) ? "0" : realGrade;
                map.put("C_GRADE", realGrade);
                /** 是否有礼包 */
                Integer has = (Integer) this.getSqlMapClientTemplate().queryForObject("ek_gameApp.hasGift", id);
                if (has == 0) {
                    map.put("C_HASGIFT", "0");
                } else {
                    map.put("C_HASGIFT", "1");
    ...
    public List<Map<String, Object>> gameRecommend() {
        return this.getSqlMapClientTemplate().queryForList("ek_gameApp.gameRecommend");
    }
    ...
    public Integer userComments(Long uid, Long gid, Date date, String comment,
        ...
        try {
            getSqlMapClientTemplate().queryForObject("ek_gameApp.procUserComments", parameterObject);
            ...
            if (grade != -1) {
                ...
                if (null == userGrade || userGrade.size() <= 0)
                    // 这边返回一个Object对象，可能为null， 所以假定语句执行不异常认为是成功，返回1.
                    getSqlMapClientTemplate().insert("ek_gameApp.userRating",
                            parameterObject);
                else
                    getSqlMapClientTemplate().update("ek_gameApp.updateUserRating",
                            parameterObject);
            }
    ...
    public List<Map<String, Object>> starGameStatistics(Long gid) {
        ...
        return _queryForList("ek_gameApp.starGameStatistics", parameterObject);
    }
    ...
    public List<Map<String, Object>> getThroughTheGameIdGameReviewList(
        ...
        tempList = _queryForList("ek_gameApp.getThroughTheGameIdGameReviewList", parameterObject);
    ...
    public List<Map<String, Object>> getThroughTheGameIdGameReviewList(
        ...
        list = getSqlMapClientTemplate().queryForList("ek_gameApp.getGameReviewsByUidAndGid", parameterObject);
        ...
        _mapList = _queryForList("ek_gameApp.getThroughTheGameIdGameReviewList2", parameterObject);
        if (null != _mapList)
    ...
    public List<Map<String, Object>> getUserRating(long uid, long gid) {
        ...
        return _queryForList("ek_gameApp.getGrade", parameterObject);
    }
    ...
    public Map<String, Object> isLike(Long id, String uid, String source) {
        ...
        List<Map<String, Object>> list = _queryForList("ek_gameApp.isLike", parameterObject);
    ...
    public Integer persistentUserBehavior(Long uid, Long gid, Date date, Integer type, Long indexid, String source) {
        ...
        try {
            ...
            getSqlMapClientTemplate().insert("ek_gameApp.persistentUserBehavior2",
                    parameterObject);
    ...
    @SuppressWarnings("unchecked")
    @Override
    public List<Map<String, Object>> advertInfo(String flag) {
        try {
            Map<String, Object> paramMap = new HashMap<String, Object>();
            String start, end;
            if ("1".equals(flag)) {
            ...
            paramMap.put("ST", start);
            paramMap.put("ED", end);
            return this.getSqlMapClientTemplate().queryForList("ek_gameApp.advertInfo", paramMap);
        } catch (Exception e) {
            log.error("EKGameAppServiceImpl.advertInfo failed, e : ", e);
        }
        return null;
    }
    ...
    public List<Map<String, Object>> gameCollection(String cid, String flag, String pageindex, String pSize) {
        try {
            // 根据分类id读取表名
            String tbName = FileUtil.getConfig("tableName", "ek.tbName.".concat(cid));
            log.info("tableName : " + tbName);
            Map<String, Object> paramMap = new HashMap<String, Object>();
            paramMap.put("CID", cid);
            paramMap.put("TBNAME", tbName);
            paramMap.put("PAGE", pageindex);
            paramMap.put("PSIZE", StrUtils.isEmpty(pSize) ? "3" : pSize);
            return this.getSqlMapClientTemplate().queryForList("ek_gameApp.gameColIndex", paramMap);
        } catch (Exception e) {
            log.error("EKGameAppServiceImpl.gameCollection failed, e : ", e);
        }
        return null;
    }
    ...
    public List<Map<String, Object>> search(String gid) {
        return _queryForList("ek_gameApp.getGameDetailByGameId", Long.parseLong(gid));
    }
    ...
    public List<Map<String, Object>> searchApp(int uid, String imei, String content,
        ...
        if ("0".equals(type)) {
            return _queryForList("ek_gameApp.searchApp1", content);
        }

        List<Map<String, Object>> list = _queryForList("ek_gameApp.searchApp", content);
        /** 2016-05-10 e键v2.1 单独查询游戏评分（因为SQL语句的链接会导致下载数量为：实际下载量 * 评分表数据条数） begin*/
        if (StrUtils.isNotEmpty(list)) {
            Map<String, Object> map = list.get(0);
            String gid = StrUtils.emptyOrString(map.get("C_ID"));
            Integer grade = (Integer) getSqlMapClientTemplate().queryForObject("ek_gameApp.appGrade", gid);
            map.put("C_GRADE", grade);
        }
        /** 2016-05-10 e键v2.1 单独查询游戏评分 end*/
        return list;
    }
}
