...
public class EKMemberServiceImpl extends SqlMapClientDaoSupport implements EKMemberService {
		...
		try {
			...
			if (StrUtils.isNotEmpty(list)) {
				for (Map<String, Object> map : list) {
					...
					String order = StrUtils.emptyOrString(map.get("C_ORDER"));
					objArr[0] = uid;
					objArr[1] = aid;
					objArr[2] = order;
					objArr[3] = new Date();
	...
	@Override
	// 根据供应商代码获取供应商名称
	public String getNameBySupCode(String sup) {
		return (String) getSqlMapClientTemplate().queryForObject("ek_member.getNameBySupCode", sup);
	}
}
