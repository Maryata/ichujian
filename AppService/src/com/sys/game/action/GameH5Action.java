 ...
 */
@Component
public class GameH5Action extends GameBaseAction {

	private static final long serialVersionUID = 2020814169379302340L;

	@Autowired
	private GameH5Service gameH5Service;
	...
	// h5游戏分类列表  
  	public String h5CategoryList(){
  		...
  		log.info("into GameH5Action.h5CategoryList");
  		try {
  			// h5游戏分类列表
 			List<Map<String, Object>> list = gameH5Service.h5CategoryList();
 			reqMap.put("list", list);
 			reqMap.put("status", "Y");
  		} catch (Exception e) {
  			reqMap.put("status", "N");
  			reqMap.put("info", "1003," + e.getMessage());
  			log.error("GameH5Action.h5CategoryList failed,e : " + e);
  		}
  	...
  	public String h5Barrage(){
 		...
 		String comment = this.getParameter("comment");// 弹幕内容
  		log.info("into GameH5Action.h5Barrage");
  		...
  		try {
  			if (StrUtils.isEmpty(uid) || StrUtils.isEmpty(gid) || 
 				...
 				// 保存用户行为
 	 			gameH5Service.saveH5Barrage(uid, gid, type, source);
 	 			// 推送消息
...
// 	 			log.info("r---------" + r.getData().toString());
  			...
  			reqMap.put("status", "N");
  			reqMap.put("info", "1003," + e.getMessage());
  			log.error("GameH5Action.h5Barrage failed,e : " + e);
  		}
  		out = JSONObject.fromObject(reqMap).toString();
  		return "success";
  	}
  	...
  	public String recentlyPlaying(){
  		...
  		log.info("into GameH5Action.recentlyPlaying");
  		...
  		try {
  			if (StrUtils.isEmpty(uid) || StrUtils.isEmpty(uid)) {
 				...
 				reqMap.put("info", "1001");
 	 			...
 	 			// 最近在玩的游戏
 				List<Map<String, Object>> list = gameH5Service.recentlyPlaying(uid, type);
  				reqMap.put("list", list);
  				reqMap.put("status", "Y");
 			}
  		} catch (Exception e) {
  			reqMap.put("status", "N");
  			reqMap.put("info", "1003," + e.getMessage());
  			log.error("GameH5Action.recentlyPlaying failed,e : " + e);
  		}
  		out = JSONObject.fromObject(reqMap).toString();
  		return "success";
  	}
  	...
  	// 根据分类id获取具体某一分类中的游戏(H5)
   	public String h5GamesInCategory(){
   		...
   		log.info("into GameH5Action.h5GamesInCategory");
   		log.info("cid = " + cid + "pageindex" + pageindex);
   		try {
   			// 分类中的游戏
   			List<Map<String, Object>> list = gameH5Service.h5GamesInCategory(cid,pageindex);
   			reqMap.put("list", list);
   			reqMap.put("status", "Y");
   		} catch (Exception e) {
   			reqMap.put("status", "N");
   			reqMap.put("info", "1003," + e.getMessage());
   			log.error("GameH5Action.h5GamesInCategory failed,e : " + e);
   		}
   		out = JSONObject.fromObject(reqMap).toString();
   		return "success";
   	}
	...
	public String searchH5() {
		...
		if ( StringUtil.isEmpty( imei ) || StringUtil.isEmpty( content ) ) {
			...
			try {
				...
				list = gameH5Service.searchH5(uid, imei, content, date, type);
	...
	public String getH5GameById() {
		...
		if ( StringUtil.isEmpty( id ) ) {
			...
			try {
				objectMap = gameH5Service.getH5GameById(id);
