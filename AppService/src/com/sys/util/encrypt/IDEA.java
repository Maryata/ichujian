...
public class IDEA {
	...
	public byte[] getKey(String key) throws UnsupportedEncodingException {
		int len1 = key.length();
		if (len1 >= 16) {
	 ...
	 * @return
	 */
	public String getEncString(String strMi) {
		String strMing = "";
		try {
			return byte2hex(IdeaEncrypt(bytekey, strMi.getBytes("UTF-8"), true));
		} catch (Exception e) {
			e.printStackTrace();
		}
		return strMing;
	...
	public String getDesString(String strMi) {
		String strMing = "";
		try {
			String tmp = new String(IdeaEncrypt(bytekey,hex2byte(strMi.getBytes("UTF-8")), false),"UTF-8");
			int len1 = tmp.length();
	...
	public static void main(String[] args) throws UnsupportedEncodingException {
		...
		idea.getKey("CXZCZX");// 生成密匙

		String strEnc = idea.getEncString("123456789asdas0张三");// 加密字符串,返回String的密文
