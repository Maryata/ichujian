...
//		}
	...
	private static X509Certificate initCert(String path) {
        String _path = CertUtil.class.getResource("").getPath();
        path = _path.substring(0,_path.lastIndexOf('/') - 3).concat(path);
	...
	private static void initValidateCertFromDir() {
		...
		try {
			cf = CertificateFactory.getInstance("X.509");
			String path = CertUtil.class.getResource("").getPath();
            path = path.substring(0,path.lastIndexOf('/') - 3).concat(dir);
...
//				ks = KeyStore.getInstance(type, "BC");
            ...
            String path = CertUtil.class.getResource("").getPath();
            path = path.substring(0,path.lastIndexOf('/') - 3).concat(pfxkeyfile);
			LogUtil.writeLog("Load RSA CertPath=[" + path + "],Pwd=["
					+ keypwd + "]");
