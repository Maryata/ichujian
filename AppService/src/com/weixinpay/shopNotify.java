            ...
            if ("SUCCESS".equals(return_code)) {
                ...
                if ("SUCCESS".equals(result_code)) {
                    ...
                    if(sign.equals(root.elementText("sign"))) {
                        ...
                        // 更新订单信息
                        // 记录微信支付日志
                        Map<String,String> r = shopService.updatePayStatus(parameterMap, "1","1");//(参数,支付类型,支付状态)支付
                        resultRoot.addElement("return_code").addCDATA(r.get("return_code"));
                        resultRoot.addElement("return_msg").addCDATA(r.get("return_msg"));
                    } else {
                        Map<String,String> r = shopService.updatePayStatus(parameterMap, "1","3");//(参数,支付类型,支付状态)支付
                        resultRoot.addElement("return_code").addCDATA("FAIL");
                        resultRoot.addElement("return_msg").addCDATA("支付失败");
                    }
                } else {
                    String r_err_code = root.elementText("err_code");

                    shopService.logWeChartPay(parameterMap);
