...
//        指定支付方式	limit_pay	否	String(32)	no_credit	no_credit--指定不能使用信用卡支付
        ...
        // 到数据库查询 商品信息
        List<Map<String,Object>> comboList = feiyuCloudService.getComboById(id);
        if(comboList != null) {
            Map<String,Object> map = comboList.get(0);
            price = ((BigDecimal)(map.get("C_PRICE"))).floatValue();
            total_fee = String.valueOf((int)(price * 100));
            time = ((BigDecimal)map.get("C_TIME")).longValue();
            if(StringUtils.isEmpty(body)) {
                body = String.valueOf(map.get("C_NAME"));
            }
        }
        ...
        root.addElement("notify_url").setText(WeixinPayConfig.NOTIFY_URL);
...
//                String r_sign = resultRootElement.element("sign").getText();
                ...
                if ("SUCCESS".equals(result_code)) {
                    ...
                    // 写入订单信息
                    // 支付类型1：微信 2:支付宝
                    feiyuCloudService.insertOrder(out_trade_no,fyAccountId,String.valueOf(price),String.valueOf(time),"1");
                } else {
