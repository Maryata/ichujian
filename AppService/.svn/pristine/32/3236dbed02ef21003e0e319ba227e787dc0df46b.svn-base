...
public class EKTaskServiceImpl extends SqlMapClientDaoSupport implements EKTaskService {
    ...
    public void reward(String uid, String taskType, String taskId, String itemId) {
        if (!"26".equals(taskId) && !hasDoneTask(uid, taskType, taskId, itemId)) {
            // 如果完成过任务，不执行奖励
            return;
        }
        String strDate = "";
    ...
    public boolean hasDoneTask(String uid, String taskType, String taskId, String itemId) {
        ...
        switch (taskType) {
            ...
            case "2" : // 每日随机任务
                ...
                paramMap.put("STRDATE", strDate);
                // 查询该任务是否是每日随机任务
                Integer randomTask = (Integer) getSqlMapClientTemplate().queryForObject("ekeyTask.isRandomTask", paramMap);
                if (randomTask > 0) {
                    result = (Integer) getSqlMapClientTemplate().queryForObject("ekeyTask.haveCompletedRandom", paramMap);
                } else {
                    // 如果不是每日随机任务，也当作做过该任务，这样就不会记奖励积分和录完成任务
                    return false;
                }
                break;
            case "3" : // 特殊任务
                if ("24".equals(taskId)) {// 任务：4号键设置苏宁易购并打开  单独处理
                    // 查询包名对应的键位APP信息--此时的itemId是jar包名
                    List<Map<String, Object>> keyAppInfo = getSqlMapClientTemplate().queryForList("ekeyTask.getKeyAppInfo", paramMap);
                    if (StrUtils.isNotEmpty(keyAppInfo)) {
                        Map<String, Object> map = keyAppInfo.get(0);
                        String appId = StrUtils.emptyOrString(map.get("C_ID"));
                        // itemId为App的id
                        paramMap.put("ITEMID", appId);
                    }

                }
                // 查询该任务是否是特殊任务
                List<Map<String, Object>> specialTask = getSqlMapClientTemplate().queryForList("ekeyTask.isSpecialTask", paramMap);
                // 如果是特殊任务，则判断是否做过
                if (StrUtils.isNotEmpty(specialTask)) {
                    String c_sdate = specialTask.get(0).get("C_SDATE").toString();
                    String c_edate = specialTask.get(0).get("C_EDATE").toString();
                    paramMap.put("SDATE", c_sdate.substring(0, c_sdate.length()-2));// 去掉末尾的毫秒
                    paramMap.put("EDATE", c_edate.substring(0, c_edate.length()-2));
                    result = (Integer) getSqlMapClientTemplate().queryForObject("ekeyTask.haveCompletedSpecial", paramMap);
                } else {
                    // 如果不是特殊任务，也当作做过该任务，这样就不会记奖励积分和录完成任务
                    return false;
                }
                break;
        }
