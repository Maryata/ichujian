...
public class AppServiceImpl extends SqlMapClientDaoSupport implements AppService {
	...
	public List<Map<String, Object>> getSubBrandByBrandId(String id) {
		...
		param.put("ID", id);
		return getSqlMapClientTemplate().queryForList("soft_sql1.getSubBrandByBrandId", param);
	}
	...
	@Override
	// 记录免费换膜行为
	public void recExchange(String imei, String uid) {
		Map<String, Object> map = new HashMap<>();
		map.put("IMEI", imei);
		map.put("UID", uid);
		getSqlMapClientTemplate().update("soft_sql1.recExchange", map);
	}

	@Override
	// 查询是否有过“免费购膜”行为
	public int hasExchanged(String imei) {
		return (Integer) getSqlMapClientTemplate().queryForObject("soft_sql1.hasExchanged", imei);
	}

	@Override
	// 根据手机型号获取型号中文名
	public List<Map<String, Object>> getNameByModel(String model) {
		return getSqlMapClientTemplate().queryForList("soft_sql1.getNameByModel", model);
	}
