...
public class EKGameBaseService extends SqlMapClientDaoSupport {
	...
	protected void userAction(List<Map<String, Object>> list,String uid){
		if(StrUtils.isNotEmpty(list)){
			for (Map<String, Object> map : list) {
				...
				map.put("GOT", got);
				// 查询礼包码数量
				List<Map<String, Object>> cnt = this.getSqlMapClientTemplate().queryForList("ek_gameGift.giftCodesCnt",gid);
				...
				if(StrUtils.isNotEmpty(gid)){
					...
					List<Map<String, Object>> codeList =
							this.getSqlMapClientTemplate().queryForList("ek_gameGift.hasGotGame", paramMap);
					if(StrUtils.isNotEmpty(codeList) && codeList.size()>0){
						...
						List<Map<String, Object>> drawCodeList =
								this.getSqlMapClientTemplate().queryForList("ek_gameGift.usersGiftCode", paramMap);
	...
	protected void _processGrade(Map<String, Object> param,
		...
		if(null != param) { // 用户没有评分
			// 当前页用户对游戏的评分列表
			_gradeList = _queryForList( "ek_gameApp.getGrade", param );
		}
