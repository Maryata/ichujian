...
//		}
	...
	private List<Catalog> loadCatalogs2(String type){
		...
		if(catalogsList!=null){
			Map<String, Catalog> map = Maps.newLinkedHashMap();
...
//	}
	...
	private List<Product> loadProducts(int type) {
		...
		List<Product> plist =  productService.selectList(p);
		Map<String,String> plines = systemManager.getSystemDicts("account_purchaseType");
		//设置 主分类和子分类;
		Catalog temp = null;
		for(Product p1 : plist){
			temp = systemManager.getCatalogsMap().get(p1.getCatalogID());
			if(null !=temp){
				p1.setChildrenCatalogName(temp.getName());
				p1.setChildrenCatalogCode(temp.getCode());
				temp = systemManager.getCatalogsMap().get(temp.getPid());
				if(null !=temp){
					p1.setMainCatalogCode(temp.getCode());
					p1.setMainCatalogName(temp.getName());
				}
			}
			if(plines.containsKey(p1.getPline()))
				p1.setPlineName(plines.get(p1.getPline()));
		}
		return plist;
	}
