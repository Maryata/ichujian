	...
	<div style="text-align: right; border: 0;padding: 4px 12px;" class="pageDiv">
		<@pg.pager url="${pager.pagerUrl}" items=pager.total
			...
			maxPageItems=pager.pageSize maxIndexPages=10 isOffset=true>
                ...
                <@pg.param name="cc"/>
			<@pg.first>
				<a href="javascript:page('${pageUrl}')" class="pageLink">首页</a>
			</@pg.first>
			<@pg.prev>
				<a href="javascript:page('${pageUrl}')" class="pageLink">上一页</a>
			</@pg.prev>
			<@pg.pages>
            <#if currentPageNumber==pageNumber>
                <span class="selectPageLink">${pageNumber}</span>
                <#else >
                    <a href="javascript:page('${pageUrl}')" class="pageLink">${pageNumber}</a>
            </#if>
			</@pg.pages>
			<@pg.next>
				<a href="javascript:page('${pageUrl}')" class="pageLink">下一页</a>
			</@pg.next>
			<@pg.last>
				<a href="javascript:page('${pageUrl}')" class="pageLink">尾页</a>
			</@pg.last>
		</@pg.pager>
	</div>
