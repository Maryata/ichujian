...
//	private String productTotal;//商品总金额
	...
	public void totalCacl(List<Product> productList){
		...
		boolean isAccType1 = false;
		if(StrUtils.isNotEmpty(acc) && "1".equals(acc.getAccType())){
			isAccType1 = true;
		}
		double _amount = 0;
		int _quantity = 0;
...
//			_productTotal += Double.valueOf(p.getNowPrice()) * p.getBuyCount();
			_amount += Double.valueOf(isAccType1 ? p.getNowPrice() : p.getPrice()) * p.getBuyCount();
			_quantity ++;
		}
