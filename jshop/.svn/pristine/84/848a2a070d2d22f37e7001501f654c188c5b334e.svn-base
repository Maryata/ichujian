...
Page.prototype.view = function(){
	...
	conf.allpage = 'allpage' in conf ? conf.allpage : false;
    ...
    //总页数大于连续分页数，且当前组最大页小于总页，输出尾页 && conf.last
	...
	var allpageView = "";
	if(conf.allpage) allpageView= "共"+conf.pages+"页&nbsp;";
    ...
    return '<div name="laypage'+ laypage.v +'" class="laypage_main laypageskin_'+ (conf.skin ? (function(skin){
        ...
        return conf.skip 
        ? '<span class="laypage_total"><label>'+allpageView+'&#x5230;&#x7B2C;</label><input type="number" min="1" onkeyup="this.value=this.value.replace(/\\D/, \'\');" class="laypage_skip"><label>&#x9875;</label>'
