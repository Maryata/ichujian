...
<@page.pageBase currentMenu=(e.type=="p")?string("商品目录","文章分类") currentListUrl="/manage/catalog/selectList?init=y&type=p">
	...
	<form action="${basepath}/manage/catalog" theme="simple" id="form" name="form">
		...
		<table class="table table-bordered" style="width: 95%;margin: auto;">
			...
			<tr>
				...
				<td style="text-align: left;"><input type="text"  value="${e.name!""}" name="name"  id="name" data-rule="名称;required;name;" size="40" maxlength="35"/></td>
			</tr>
				...
				<td style="text-align: left;">
				    <select name="mtype" id="mtype">
             		  <#list materialMap?keys as key>
             			<option value="${key}" <#if e.mtype?? && e.mtype==key>selected</#if> >
             				${materialMap[key]}
         				</option>
             		 </#list>	
             	  </select>
...
<#--<%-- 						<td style="text-align: left;"><input type="text"  value="${e.code!""}" name="code"  data-rule="编码;required;code;" size="20" maxlength="20" id="code" /></td> --%>-->
			...
			<tr>
				...
				<td style="text-align: left;"><input type="text"  value="${e.order1!""}" name="order1"  data-rule="顺序;required;integer;order1;" size="20" maxlength="20" id="order1" /></td>
			</tr>
