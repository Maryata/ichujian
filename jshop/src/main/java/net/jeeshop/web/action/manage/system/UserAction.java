...
//	}
	...
	public String login(HttpSession session,@ModelAttribute("e") User e, ModelMap model) throws Exception {
		...
		String errorMsg;
		if (session.getAttribute(ManageContainer.manage_session_user_info) != null) {
			/*if(null!=returl && returl.length()>5 && returl.indexOf(request.getContextPath())>-1){
		...
		//查询页面跳转;
		/*if(null!=returl && returl.length()>5 && returl.indexOf(request.getContextPath())>-1 && returl.indexOf("selectList")>-1){
			return "redirect:"+returl;
		}*/
		return "redirect:/manage/user/home";
	}
...
//			}
	...
	public String insert(HttpServletRequest request, @ModelAttribute("e") User user, RedirectAttributes flushAttrs) throws Exception {
    	return save0(request, user, flushAttrs);
	}
	...
	private String save0(HttpServletRequest request, User e, RedirectAttributes flushAttrs) throws Exception {
		...
		if(StringUtils.isBlank(e.getId())){//添加
			...
			/** 2015-6-19 修改：添加当前用户所属部门  begin*/
	    	String did = request.getParameter("did");
	    	userService.insertDept(e.getId(),did);
	    	/** 2015-6-19 修改：添加当前用户所属部门  begin*/
		}else{//修改
...
//				throw new RuntimeException("操作非法！");
			...
			/** 2015-6-19 修改：修改当前用户所属部门  begin*/
	    	String did = request.getParameter("did");
	    	userService.updateDept(e.getId(),did);
	    	/** 2015-6-19 修改：修改当前用户所属部门  begin*/
		}
	...
	public String unique(@ModelAttribute("e") User e, HttpServletResponse response) throws IOException{
		...
		}else if(StringUtils.isNotBlank(e.getUsercode())){//验证工号否被占用
			logger.error("验证工号是否被占用");
			User user = new User();
			user.setUsercode(e.getUsercode());
			user = userService.selectOneByCondition(user);
			if(user==null){
				//数据库中部存在此编码
				return "{\"ok\":\"工号可以使用!\"}";
...
//		errorMsg = "旧密码输入错误，修改密码失败!";
	...
	public String toAdd(@ModelAttribute("e")User user, ModelMap model) throws Exception {
        ...
        /** 2015-6-19 修改：添加部门查询  begin*/
        model.addAttribute("deptList", deptService.loadRoot(null));
        /** 2015-6-19 修改：添加部门查询  end*/
		return page_toAdd;
	}
	@Override
...
//		}
        ...
        model.addAttribute("deptList", deptService.loadRoot(null));
        // 添加当前用户所属部门id
        model.addAttribute("deptId", userService.getDept(e));
        /** 2015-6-19  end*/
    ...
    @ResponseBody
    public String updatePass(User e, RedirectAttributes flushAttrs){
    	JSONObject json = new JSONObject();
    	//修改密码
    	if(StrUtils.isEmpty(e.getId()) && StrUtils.isEmpty(e.getPassword()) &&  StrUtils.isEmpty(e.getNewpassword2())){
    		json.put("msg", "密码不能为空");
    		json.put("status", "0");
    	}else if(!e.getPassword().equals(e.getNewpassword2())){
    		json.put("msg", "两次输入的密码不一致！");
    		json.put("status", "0");
    	}else{
    		int row=getService().update(e);
    		if(row>0){
    			json.put("msg", "修改成功！");
        		json.put("status", "1");
    		}else{
