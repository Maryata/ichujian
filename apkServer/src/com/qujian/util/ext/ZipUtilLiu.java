...
public class ZipUtilLiu {  
    ...
    /**解压源文件目录*/  
    private static final String SOURCE_PATH_NAME = "/mokey/APK/source/";      
    /**打包目录*/  
    private static final String TARGET_PATH_NAME = "/mokey/APK/target/";  
    /**签名目录*/  
    private static final String RESULT_PATH_NAME = "/mokey/APK/result/";  
    /**JDK BIN 目录*/  
    private static final String JDK_BIN_PATH = "/usr/java/jdk1.7.0_51/bin";  
    /**密钥 目录*/  
    private static final String SECRET_KEY_PATH = "/mokye/APK/mokey_android.android";  
    /**密钥 名称*/  
    private static final String SRC_PATH = "/mokey/APK/ichujian_android.apk";  
    ...
    */    
    ...
    public void exec()throws Exception{  
        ...
        //解压源文件保存路径  
        String sourcePath = buffer.append(SOURCE_PATH_NAME).  
                                append(prefixName).append("/").toString();  
        System.out.println("sourcePath ==== "   +  srcPath);
        System.out.println("sourcePath ==== "   +  sourcePath); 
        //------解压  
        unZip(srcPath, sourcePath);  
        System.out.println("========uzip suessce");  
        //------删除解压后的签名文件  
        String signPathName = sourcePath+SIGN_PATH_NAME; 
        System.out.println("delete path ==== "   +  signPathName); 
        ...
        System.out.println("========delete suessce");  
        //------修改内容  
        buffer.setLength(0);  
        String path = buffer.append(SOURCE_PATH_NAME)  
        ...
        String targetPath = TARGET_PATH_NAME;  
        //判断创建文件夹  
        File targetFile = new File(targetPath);  
        if(!targetFile.exists()){  
            targetFile.mkdir();  
        }  
        compress(SOURCE_PATH_NAME+prefixName,targetPath+fileName);  
        System.out.println("========"+parentPath+SOURCE_PATH_NAME+"======"+targetPath+fileName);
        ...
        System.out.println("dabao："+(t11-t0)/1000.0); 
        //组合签名命令  
        long t12 =System.currentTimeMillis(); 
       buffer.setLength(0);  
     /*  // jarsigner -verbose -keystore android.keystore my_apk_sign.apk my_apk_unsign.apk alias_name
        buffer.append("cmd.exe /c jarsigner -verbose -keystore ")  
        .append(SECRET_KEY_PATH).append(SECRET_KEY_NAME).append(" ")  
    	...
    	//StringBuffer buffer = new StringBuffer();  
		//组合签名命令  
       String keyFile = RESULT_PATH_NAME+fileName;
       String target = TARGET_PATH_NAME+fileName;
        buffer.setLength(0);  
        buffer.append("jarsigner ")  
        .append("-keystore ").append(SECRET_KEY_PATH)
        .append(" -storepass mokey_android ")  
        .append("-signedjar ").append(keyFile).append(" ")    //签名保存路径应用名称  
        .append(target).append(" ")    //打包保存路径应用名称
        .append("cn"); 
        //利用命令调用JDK工具命令进行签名  
        ...
        System.out.println("qianming："+(t13-t12)/1000.0);
        System.out.println("总耗时："+(t2-t1)/1000.0);
    }  
} 
