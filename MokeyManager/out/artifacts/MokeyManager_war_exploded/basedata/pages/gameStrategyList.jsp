...
function infoList(){
	...
	// 查询总数
	$.post("${pageContext.request.contextPath}/basedata/gameStrategyAction!getTotal.action",{gid:gid,name:name,gName:gName},
		function(data){
			var json = $.parseJSON(data);
			$("#total").val(json);
			// 分页查询
			getGameStrategyPage("${pageContext.request.contextPath}/basedata/gameStrategyAction!gameStrategyList.action",gid,name,gName);
	});
	// 查询所有
	$.post("${pageContext.request.contextPath}/basedata/gameStrategyAction!gameStrategyList.action",{gid:gid,name:name,gName:gName},
		function(data){
			json = $.parseJSON(data);
			$("#mytable").empty();
			$(json).each(function(){
...
function deleteInfo(id){
    ...
    if(flag){
		$.post("${pageContext.request.contextPath}/basedata/gameStrategyAction!deleteGameStrategy.action",
				{id:id},function(data){
...
function addInfo(id){
	if(id==null || id==""){// 没有id，添加
		$("#form1").attr("action","${pageContext.request.contextPath}/basedata/gameStrategyAction!toGameStrategyAdd.action");
		$("#form1").submit();
	}else{// 有id，编辑
		$("#form1").attr("action","${pageContext.request.contextPath}/basedata/gameStrategyAction!toGameStrategyEdit.action");
		$("#editId").val(id);
		$("#form1").submit();
	}
}
