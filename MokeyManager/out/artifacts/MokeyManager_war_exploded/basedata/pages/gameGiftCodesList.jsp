...
function codesList(){
	...
	// 查询总数
	$.post("${pageContext.request.contextPath}/basedata/gameGiftAction!getCodesListByGid.action",{gid:gid,state:state,sDate:sDate,eDate:eDate},
		function(data){
			...
			if(count != 0){
				// 分页查询
				getGameGiftCodesPage("${pageContext.request.contextPath}/basedata/gameGiftAction!getCodesListByGid.action",gid,state,sDate,eDate);
			}
	});
	// 查询所有
	$.post("${pageContext.request.contextPath}/basedata/gameGiftAction!getCodesListByGid.action",{gid:gid,state:state,sDate:sDate,eDate:eDate},
		function(data){
			...
			$(list).each(function(){
				...
				_str += "<td align='center' name='userId'>"+(this.USERID||'')+"</td>";
...
function delCode(id){
    ...
    var gid = $("#gid").val();// 获取礼包id
    if(flag){
		$.post("${pageContext.request.contextPath}/basedata/gameGiftAction!delCode.action",
...
function addInfo(id){
	if(id==null || id==""){// 没有id，添加
		$("#form1").attr("action","${pageContext.request.contextPath}/basedata/gameGiftAction!toGameGiftAdd.action");
		$("#form1").submit();
	}else{// 有id，编辑
		$("#form1").attr("action","${pageContext.request.contextPath}/basedata/gameGiftAction!toGameGiftEdit.action");
		$("#editId").val(id);
		$("#form1").submit();
	}
}
...
function giftCodes(id){
	$("#form1").attr("action","${pageContext.request.contextPath}/basedata/gameGiftAction!toGiftCodesList.action");
	$("#editId").val(id);
	$("#form1").submit();
}
// 批量删除
function batchDel(){
	...
	if(flag){
	  	...
	  	$("#mytable input:checked").each(function(){
	   	...
	   	var id = ids.toString();
	   	$.post("${pageContext.request.contextPath}/basedata/gameGiftAction!batchDel.action",
	   		{id:id,gid:gid},function(data){
				window.location.reload();
	    }); 
    } 
...
function exportExcel(){
	...
	$("#form1").attr("action","${pageContext.request.contextPath}/basedata/gameGiftAction!exportExcel.action");
	$("#form1").submit();
}
</script>
</html>
