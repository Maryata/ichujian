...
function infoList(){
	...
	// 查询总数
	$.post("${pageContext.request.contextPath}/basedata/gameActAction!getTotal.action",{gid:gid,name:name,gName:gName},
		function(data){
			var json = $.parseJSON(data);
			$("#total").val(json);
			// 分页查询
			getGameActPage("${pageContext.request.contextPath}/basedata/gameActAction!GameActList.action",gid,name,gName);
	});
	// 查询所有
	$.post("${pageContext.request.contextPath}/basedata/gameActAction!GameActList.action",{gid:gid,name:name,gName:gName},
		function(data){
			json = $.parseJSON(data);
			$("#mytable").empty();
			$(json).each(function(){
...
function _users(id) {
	if(id) {
		window.location.href = '${pageContext.request.contextPath}/basedata/gameActAction!users.action?aid=' + id;
	}
	// 没有ID是错误的的
}
...
function deleteInfo(id){
    ...
    if(flag){
		$.post("${pageContext.request.contextPath}/basedata/gameActAction!deleteGameAct.action",
				{id:id},function(data){
...
function addInfo(id){
	if(id==null || id==""){// 没有id，添加
		$("#form1").attr("action","${pageContext.request.contextPath}/basedata/gameActAction!toGameActAdd.action");
		$("#form1").submit();
	}else{// 有id，编辑
		$("#form1").attr("action","${pageContext.request.contextPath}/basedata/gameActAction!toGameActEdit.action");
		$("#editId").val(id);
		$("#form1").submit();
	}
}
