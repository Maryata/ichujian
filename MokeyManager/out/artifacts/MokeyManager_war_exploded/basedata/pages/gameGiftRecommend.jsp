    ...
    <td>
      <table width="100%"  border="0" cellspacing="1" cellpadding="2">
        <tr> 
         ...
         <td align="center" class="biaodan-q">
           <input name="Button" type="button" class="butt" value="查 询" onclick="recommendGiftList()" />
           </td>
        </tr>
      </table>
    </td>
...
<input type="hidden" id="recommendTotal" />
<div id="page22"></div>
</form>

<script>
// 全局数组，用来存放移除出当前推荐的礼包id
...
function giftList(){
	...
	// 查询未作为推荐的礼包
	$.post("${pageContext.request.contextPath}/basedata/gameGiftRecommendAction!getAllGameGift.action",{name:name,giftCate:giftCate},
		function(data){
			var json = eval("("+data+")");
			...
			if(json != 0){
				...
				// 所有礼包
				getGiftPage("${pageContext.request.contextPath}/basedata/gameGiftRecommendAction!getAllGameGift.action",name,giftCate);
			}
	});
	$.post("${pageContext.request.contextPath}/basedata/gameGiftRecommendAction!getAllGameGift.action",{name:name,giftCate:giftCate},
		function(data){
...
// 当前推荐的所有礼包
function recommendGiftList(){
	var name = $("#cateGiftName").val();
	// 查询当前推荐的礼包总数
	$.post("${pageContext.request.contextPath}/basedata/gameGiftRecommendAction!recommendGiftList.action",{name:name},
		function(data){
			var json = eval("("+data+")");
			var count = json[0].count;// 数据总数
			$("#page22").empty();
			if(json != 0){
				setTotal(count,"recommendTotal");
				// 当前推荐的礼包
				currCateGiftPage("${pageContext.request.contextPath}/basedata/gameGiftRecommendAction!recommendGiftList.action",name);
			}
	});
	$.post("${pageContext.request.contextPath}/basedata/gameGiftRecommendAction!recommendGiftList.action",{name:name},
		function(data){
...
// 分页查询所有礼包
function getGiftPage(url,name,cid) {
	laypage({
	    cont: 'page11',
	    ...
	    jump: function(e, first){ //触发分页后的回调
	        if(!first){ //一定要加此判断，否则初始时会无限刷新
	           $.post(url,{page:e.curr,name:name,cid:cid},
					function(data){
						json = eval("("+data+")");
						var list = json[0].list;
						$("#table1").empty();
...
// 分页查询当前推荐的礼包
function currCateGiftPage(url,name) {
	laypage({
	    cont: 'page22',
	    pages: document.getElementById("recommendTotal").value,
	    curr: function(){ //通过url获取当前页，也可以同上（pages）方式获取
	        var page = location.search.match(/page=(\d+)/);
	        return page ? page[1] : 1;
	    }(), 
...
function submitt(){
	...
	$.post("${pageContext.request.contextPath}/basedata/gameGiftRecommendAction!handle.action",{gid:gid,removeGid:removeGid,order:order},function(data){
