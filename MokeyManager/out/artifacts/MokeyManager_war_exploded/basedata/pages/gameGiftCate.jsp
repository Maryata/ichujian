...
<form id="form1" method="post" action="${pageContext.request.contextPath}/basedata/gameGiftCateAction!toCateAdd.action">
...
$(function(){
	// 查询应用总数
	$.post("${pageContext.request.contextPath}/basedata/gameGiftCateAction!gameGiftCateList.action",
		function(data){
			...
			if(count != 0){
				...
				// 分页显示礼包分类
				getCatePage("${pageContext.request.contextPath}/basedata/gameGiftCateAction!gameGiftCateList.action");
			}
	});
	// 查询分类(第一页)
	$.post("${pageContext.request.contextPath}/basedata/gameGiftCateAction!gameGiftCateList.action",
...
function deleteCate(cid){
    ...
    if(flag){
		$.post("${pageContext.request.contextPath}/basedata/gameGiftCateAction!delGameGiftCate.action",
...
function modifyCate(cid,cname,order,logo){
	$("#form2").attr("action","${pageContext.request.contextPath}/basedata/gameGiftCateAction!toUpdate.action"); 
	$("#editId").val(cid);
	$("#editName").val(cname);
	$("#editOrder").val(order);
	$("#editLogo").val(logo);
...
function handleCate(cid,cname){
	$("#form2").attr("action","${pageContext.request.contextPath}/basedata/gameGiftCateAction!toHandle.action"); 
	$("#editId").val(cid);
	$("#editName").val(cname);
	$("#form2").submit();
}
