...
<table width="98%" height="35"  border="0" align="center" cellpadding="1" >
  <tr>
    <td width="62.7%" class="tz_crumbs"><img src="${pageContext.request.contextPath}/images/crumbs.jpg" width="9" height="9" /> 系统信息 -&gt; 3号键管理 -&gt; 游戏礼包首页排版维护 </td>
  </tr>
</table>
...
// 定义Map，存放移除出首页的分类的排序
var orderMap = {};
// 定义Map，存放移除出首页的分类的礼包的数量
var numberMap = {};

$(function(){
	var appName = $("#appName").val();// 礼包名称
	var cateName = $("#cateName").val();// 分类中礼包的名称
	cateList(appName);// 查询礼包
...
function cateList(appName){
	...
	$.post("${pageContext.request.contextPath}/basedata/gameGiftIndexAction!cateList.action",{name:appName},
		function(data){
			...
			if(count != 0){
				...
				// 分页查询所有分类
				getAppPage("${pageContext.request.contextPath}/basedata/gameGiftIndexAction!cateList.action",appName);
			}
	});
	// 查询分类(第一页)
	$.post("${pageContext.request.contextPath}/basedata/gameGiftIndexAction!cateList.action",{name:appName},
...
function indexCateList(cateName){
	...
	$.post("${pageContext.request.contextPath}/basedata/gameGiftIndexAction!indexCateList.action",{name:cateName},
		function(data){
			var json = $.parseJSON(data);
			var count = json[0].count;// 数据总数
			$("#page22").empty();
			if(count != 0){
				...
				// 当前首页显示的分类
				getIndexCateList("${pageContext.request.contextPath}/basedata/gameGiftIndexAction!indexCateList.action",cateName);
			}
	});
	// 查询首页显示的分类(第一页)
	$.post("${pageContext.request.contextPath}/basedata/gameGiftIndexAction!indexCateList.action",{name:cateName},
...
function submitt(){
	...
	var number = new Array();// 每个分类的礼包的数量
	// 获取下面表的所有礼包id
	...
	if(flag){
		$.post("${pageContext.request.contextPath}/basedata/gameGiftIndexAction!handle.action",{id:id,removeId:removeId,order:order,number:number},function(data){
