(function(global, $) {
	...
	var list = function(queryParam) {
		...
		$.ajax({
			type : 'GET',
			url : basePath + '/basedata/gameTaskAction!ajaxList.action',
			data : queryParam,
			dataType : 'json',
	...
	GameTask.save = function(obj) {
		...
		var formAction = basePath + '/basedata/gameTaskAction!ajaxSave.action';
		$.ajax({
			url         : formAction,
	...
	GameTask.del = function(id) {
		if(id) {
			if(confirm('确定删除本条数据吗？')) {
				$.ajax({
					type : 'POST',
					url : basePath + '/basedata/gameTaskAction!ajaxDelete.action',
					data : {'c_id' : id},
					dataType : 'json',
					success : function( data ) {
						if( data && data.status === 'Y') {
