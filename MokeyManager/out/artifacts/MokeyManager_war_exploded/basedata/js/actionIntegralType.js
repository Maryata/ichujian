UPDATE_TAGS="";
Ext.actionIntegralType = function() {
	return {
		//定义下拉框
		priData : {
			categoryData : [{key:'1',value:'是'},{key:'0',value:'否'}]
		},
		formId : 'actionIntegralType_formId',
		gridId : 'actionIntegralType_gridId',
		addWinId : 'actionIntegralType_addWinId',
		textwidth : 120,
		init : function(){
		...
		priSaveFun : function(){
			...
			saveParam.c_integral = dirfrom.findField('c_integral').getValue();
			...
			//activityIntegralType!*
			dirfrom.submit({
				method : 'post',
				url : rootPath+"basedata/actionIntegralType!saveActionIntegralTypeInfo.action",
				waitMsg : 'save ing...',
		...
		priCreateWinFun : function(){
			...
			function checkExits(name,value,id) {
					...
					conn.open("post", rootPath+"basedata/actionIntegralType!checkActionIntegralTypeInfo.action", false);// false表示同步调用
			...
			var c_form = new Ext.form.FormPanel({
				...
				items : [{
					...
					validator : function(value) {
						//*************
						var data = checkExits("c_code", value,Ext.getCmp(_this.addWinId + 'addForm').getForm().Ext_cid);
				        //var data = checkExitsNumber("c_number", value);//dirfrom.Ext_cid = data.C_ID;
						if (!Ext.isEmpty(data.isExits) && data.isExits == '1') {
							return "编码重复";
						}
		...
		createGrid : function(){
    		...
    		var resultsGrid_store = new Ext.data.Store({ 
    			proxy : new Ext.data.HttpProxy({
    				url : rootPath+"basedata/actionIntegralType!getActionIntegralTypeList",
    				timeout : 60000
    			}),
    		...
    		var tempArray = [
               ...
               { header: "编码", sortable: true, dataIndex: 'C_CODE',width:60},
               { header: "活动类型名称", sortable: true, dataIndex: 'C_NAME',width:60},
               { header: "积分", sortable: true, dataIndex: 'C_INTEGRAL',width:60},
               { header: "按键来源", sortable: true, dataIndex: 'C_KEY',width:60},
    		  ...
    		  if (btn) {
    			...
    			if(record&&control=='delete'){
    				if(window.confirm('是否确认删除？')){
						...
						loadMask.show();
						Ext.Ajax.request({
				            url : rootPath+"basedata/actionIntegralType!deleteActionIntegralTypeInfo.action",
				            timeout : 120000,
    				...
    				dirfrom.findField('c_integral').setValue(data.C_INTEGRAL);
