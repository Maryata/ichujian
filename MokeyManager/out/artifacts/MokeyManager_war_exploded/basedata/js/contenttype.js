UPDATE_TAGS="";
Ext.contenttype = function() {
	return {
		priData : {
			categoryData : [{key:'0',value:'否'},{key:'1',value:'是'}]
		},
		formId : 'contenttype_formId',
		gridId : 'contenttype_gridId',
		addWinId : 'contenttype_addWinId',
		textwidth : 120,
		init : function(){
		...
		priSaveFun : function(){
			...
			dirfrom.submit({
				method : 'post',
				url : rootPath+"basedata/contenttype!Save.action",
				waitMsg : 'save ing...',
		...
		priCreateWinFun : function(){
			...
			var titleField = _this.createComVarField({fieldLabel:"是否显示标题",id: _this.addWinId + 'c_istitle',name:'c_istitle' , data:_this.priData.categoryData , anchor : '98%',isNoBlank:true});
            
            var functionNameStore = new Ext.data.Store({
            	//autoLoad: true,
            	proxy : new Ext.data.HttpProxy({
            		url : rootPath+"basedata/contenttype!findName.action",
            	    timeout : 60000
            	}),
			...
			function checkExits(name,id) {
				...
				conn.open("post", rootPath+"basedata/contenttype!checkName.action", false);// false表示同步调用
		...
		createGrid : function(){
    		...
    		var resultsGrid_store = new Ext.data.Store({ 
    			proxy : new Ext.data.HttpProxy({
    				url : rootPath+"basedata/contenttype!GetList.action",
    				timeout : 60000
    			}),
    			reader : new Ext.data.JsonReader({
    				totalProperty : 'count',
    		  ...
    		  if (btn) {
    			...
    			if(record&&control=='delete'){
    				if(window.confirm('是否确认删除？')){
						...
						loadMask.show();
						Ext.Ajax.request({
				            url : rootPath+"basedata/contenttype!delete.action",
				            timeout : 120000,
    				...
    				dirfrom.findField('c_istitle').setValue(data.C_ISTITLE);
    			}
    		  }   
    		},   
    		this); 
