UPDATE_TAGS="";
Ext.functioninfo = function() {
	return {
		priData : {
			categoryData : [{key:'0',value:'否'},{key:'1',value:'是'}]
		},
		formId : 'functioninfo_formId',
		gridId : 'functioninfo_gridId',
		addWinId : 'functioninfo_addWinId',
		textwidth : 120,
		init : function(){
		...
		priSaveFun : function(){
			...
			saveParam.c_pid = dirfrom.findField('c_pid').getValue();
		    saveParam.c_name = dirfrom.findField('c_name').getValue();
			saveParam.c_is_menu = dirfrom.findField('c_is_menu').getValue();
			...
			dirfrom.submit({
				method : 'post',
				url : rootPath+"system/function!SaveFunction.action",
				waitMsg : 'save ing...',
		...
		priCreateWinFun : function(){
            ...
            var typeField = _this.createComVarField({fieldLabel:"是否菜单",id: _this.addWinId + 'c_is_menu',name:'c_is_menu' , data:_this.priData.categoryData , anchor : '98%',isNoBlank:true});
			
            var functionNameStore = new Ext.data.Store({
            	//autoLoad: true,
            	proxy : new Ext.data.HttpProxy({
            		url : rootPath+"system/function!findName.action",
            	    timeout : 60000
            	}),
            ...
            functionNameStore.load({
            	callback: function(obj){
					functionNameStore.insert(0,new functionNameStore.recordType({C_ID:'0',C_NAME:'系统'}));
				}
            });
			...
			function checkExits(name,id) {
				...
				conn.open("post", rootPath+"system/function!checkName.action", false);// false表示同步调用
		...
		createGrid : function(){
    		...
    		var resultsGrid_store = new Ext.data.Store({ 
    			proxy : new Ext.data.HttpProxy({
    				url : rootPath+"system/function!GetFunctionList.action",
    				timeout : 60000
    			}),
    			reader : new Ext.data.JsonReader({
    				totalProperty : 'count',
    		...
    		var tempArray = [
               ...
               { header: "父级菜单", sortable: true, dataIndex: 'C_PID',width:20,
        	        renderer:function (value, meta, record){
            	   		if(!Ext.isEmpty(record.get('P_NAME'))){
            	   			return record.get('P_NAME');
            	   		}
            	        return value==0?'系统':value;
            	   }},
               { header: "功能名称", sortable: true, dataIndex: 'C_NAME',width:20},
               { header: "是否菜单", sortable: true, dataIndex: 'C_IS_MENU',width:10,
            	       renderer: function (value, meta, record) {
            		   return value==1?'是':'否';
            	   }},
               { header: "级别", sortable: true, dataIndex: 'C_LEVE',width:10},
               { header: "URL", sortable: true, dataIndex: 'C_URL',width:60},
               { header: "修改时间", sortable: true, dataIndex: 'C_MODITY_TIME',width:20},
               { header: "排序", sortable: true, dataIndex: 'C_ORDER',width:10},
    		  ...
    		  if (btn) {
    			...
    			if(record&&control=='delete'){
    				if(window.confirm('是否确认删除？')){
						...
						loadMask.show();
						Ext.Ajax.request({
				            url : rootPath+"system/function!deleteFunction.action",
				            timeout : 120000,
    				...
    				dirfrom.findField('c_pid').setValue(data.C_PID);      //实际值
    				if('0'==data.C_PID){
    					dirfrom.findField('c_pid').setRawValue('系统');  
    				}else
    					dirfrom.findField('c_pid').setRawValue(data.P_NAME);  //显示值
    				dirfrom.findField('c_name').setValue(data.C_NAME);
    				dirfrom.findField('c_is_menu').setValue(data.C_IS_MENU);
