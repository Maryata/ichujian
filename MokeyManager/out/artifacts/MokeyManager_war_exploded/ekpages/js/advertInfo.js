Ext.advertInfo = function() {
	return {
			...
			_this.getItemStore = function(){
				...
				var itemStores = new Ext.data.Store( {
					proxy : new Ext.data.HttpProxy( {
						url : rootPath + "basedata/ekey/eKAdvertInfo!getListByType.action",
						timeout : 60000 
					}),
		...
		priSaveFun : function(){
			...
			dirfrom.submit({
				method : 'post',
				url : rootPath+"basedata/ekey/eKAdvertInfo!saveAdvertIfo.action",
				waitMsg : 'save ing...',
...
//			adTypeStore.load();
			...
			function checkExits(appType,checkType, value,idVal) {
				...
				conn.open("post", rootPath+"basedata/ekey/eKAdvertInfo!checkAdvertIfo.action", false);// false表示同步调用
			...
			/*function checkGameIdExits(appType,checkType, value,idVal) {
				...
				var conn = Ext.lib.Ajax.getConnectionObject().conn;
				conn.open("post", rootPath+"basedata/ekey/eKAppInfo!checkAppIfo.action", false);// false表示同步调用
				conn.setRequestHeader("Content-Type","application/x-www-form-urlencoded;charset=UTF-8");
				conn.send("appType="+appType+"&checkType="+checkType+"&value="+value+"&idVal="+idVal);//
				var data = Ext.util.JSON.decode(conn.responseText);
				return data;
...
//					}
		...
		createGrid : function(){
    		...
    		var resultsGrid_store = new Ext.data.Store({ 
    			proxy : new Ext.data.HttpProxy({
    				url : rootPath+"basedata/ekey/eKAdvertInfo!getAdvertIfoList.action",
    				timeout : 60000
    			}),
    			reader : new Ext.data.JsonReader({
    				totalProperty : 'count',
    		  ...
    		  if (btn) {
    			...
    			if(record&&control=='delete'){
    				if(window.confirm('是否确认删除？')){
						...
						loadMask.show();
						Ext.Ajax.request({
				            url : rootPath+"basedata/ekey/eKAdvertInfo!deleteAdvertIfo.action",
				            timeout : 120000,
