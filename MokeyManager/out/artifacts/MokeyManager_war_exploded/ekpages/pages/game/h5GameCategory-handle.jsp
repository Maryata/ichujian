    ...
    <td>
      <table width="100%"  border="0" cellspacing="1" cellpadding="2">
        <tr> 
         ...
         <td width="60.9%" class="biaodan-q" align="left">
...
<script>
// 全局数组，用来存放移除出当前分类的H5游戏id
var removeGid = new Array();
// 定义Map，存放移除分类的H5游戏的排序
var orderMap = {};
...
function gameList(){
	...
	// 查询非当前分类的其他所有H5游戏
	$.post("${pageContext.request.contextPath}/basedata/h5GameCategoryAction!getAllGame.action",{cid:cid,name:name,gameCate:gameCate},
		function(data){
			var json = eval("("+data+")");
			...
			if(json != 0){
				...
				// 所有H5游戏
				getGamePage("${pageContext.request.contextPath}/basedata/h5GameCategoryAction!getAllGame.action",cid,name,gameCate);
			}
	});
	$.post("${pageContext.request.contextPath}/basedata/h5GameCategoryAction!getAllGame.action",{cid:cid,name:name,gameCate:gameCate},
		function(data){
...
function cateGameList(){
	...
	// 查询当前分类的H5游戏总数
	$.post("${pageContext.request.contextPath}/basedata/ekey/eKH5GameCategoryAction!getCurrCateGame.action",{cid:cid,name:name},
		function(data){
			var json = eval("("+data+")");
			var count = json[0].count;// 数据总数
			$("#page22").empty();
			if(json != 0){
				...
				// 当前分类中的H5游戏
				currCateGamePage("${pageContext.request.contextPath}/basedata/ekey/eKH5GameCategoryAction!getCurrCateGame.action",cid,name);
			}
	});
	$.post("${pageContext.request.contextPath}/basedata/ekey/eKH5GameCategoryAction!getCurrCateGame.action",{cid:cid,name:name},
		function(data){
...
function submitt(){
	...
	$.post("${pageContext.request.contextPath}/basedata/ekey/eKH5GameCategoryAction!handleCate.action",{cid:cid,gid:gid,removeGid:removeGid,order:order},function(data){
