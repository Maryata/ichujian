...
function uploadFile(btn,fileId) {
	...
	$(btn).html("uploading...");
	$.ajaxFileUpload( {
		url : "${pageContext.request.contextPath }/basedata/ekey/eKGameGiftAction!upload.action?id="+$("#newId").val(),//用于文件上传的服务器端请求地址
...
function formValid(){
	...
	if(_flag){
		$("#form1").attr("action","${pageContext.request.contextPath }/basedata/ekey/eKGameGiftAction!saveGameGift.action");
	}
	return _flag;
}
</script>
