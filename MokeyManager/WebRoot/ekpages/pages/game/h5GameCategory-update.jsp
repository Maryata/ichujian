...
function formValid(){
	...
	if(_flag){
		if($("#addFlag").val()=="1"){
			$("#form1").attr("action","${pageContext.request.contextPath }/basedata/ekey/eKH5GameCategoryAction!addGameCate.action");
		}else{
			$("#form1").attr("action","${pageContext.request.contextPath }/basedata/ekey/eKH5GameCategoryAction!updateGameCate.action");
		}
	}
	return _flag;
}
...
function uploadFile(btn,fileId,baseId) {
	...
	$(btn).html("uploading...");
	$.ajaxFileUpload( {
		url : "${pageContext.request.contextPath }/LogoUploadServlet.do?ekey=3&type=h5AppCategory/logo&id="+$("#cid").val(),//用于文件上传的服务器端请求地址
