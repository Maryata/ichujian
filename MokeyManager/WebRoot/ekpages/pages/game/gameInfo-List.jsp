...
function infoList(){
	...
	// 查询总数
	$.post("${pageContext.request.contextPath}/basedata/ekey/eKGameInfoAction!getTotal.action",{gid:gid,name:name,gName:gName},
		function(data){
			var json = $.parseJSON(data);
			$("#total").val(json);
			// 分页查询
			getGameInfoPage("${pageContext.request.contextPath}/basedata/ekey/eKGameInfoAction!gameInfoList.action",gid,name,gName);
	});
	// 查询所有
	$.post("${pageContext.request.contextPath}/basedata/ekey/eKGameInfoAction!gameInfoList.action",{gid:gid,name:name,gName:gName},
		function(data){
...
function deleteInfo(id){
    ...
    if(flag){
		$.post("${pageContext.request.contextPath}/basedata/ekey/eKGameInfoAction!deleteGameInfo.action",
				{id:id},function(data){
...
function addInfo(id){
	if(id==null || id==""){// 没有id，添加
		$("#form1").attr("action","${pageContext.request.contextPath}/basedata/ekey/eKGameInfoAction!toGameInfoAdd.action");
		$("#form1").submit();
	}else{// 有id，编辑
		$("#form1").attr("action","${pageContext.request.contextPath}/basedata/ekey/eKGameInfoAction!toGameInfoEdit.action");
		$("#editId").val(id);
		$("#form1").submit();
	}
}
