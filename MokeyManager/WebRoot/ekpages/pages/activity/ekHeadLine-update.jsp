...
function formValid(){
	...
	if(_flag){
		if($("#addFlag").val()=="1"){
			$("#form1").attr("action","${pageContext.request.contextPath }/basedata/ekey/EKHeadLineAction!addHeadLine.action");
		}else{
			$("#form1").attr("action","${pageContext.request.contextPath }/basedata/ekey/EKHeadLineAction!updateHeadLine.action");
		}
	}
	return _flag;
}
...
function uploadFile(btn,fileId,baseId) {
	...
	$.ajaxFileUpload( {// ekey : 1.首页，2.活动，3.游戏
		url : "${pageContext.request.contextPath }/LogoUploadServlet.do?ekey=2&type=activityHeadLine/logo&id="+$("#cid").val(),//用于文件上传的服务器端请求地址
...
<table width="80%"  border="0" align="center" cellpadding="0" cellspacing="1" class="tbg sortable">
	...
	<%--<input type="hidden" id="eid" name="eid" value="<s:property value="#request.eid"/>" >--%>
	<br/>
  	<tr> 
	   	<td width="5%"  height="23" class="biaodan-top" align="left">名称：</td>
		<td align="center" class="biaodan-q required" width="15%">
			<input type="text" style="border:none; width:100%; height:100%"
				   id="name" name="name" value="<s:property value="#request.name"/>" maxlength="16" >
		</td>
		<td width="5%"  height="23" class="biaodan-top" align="left">类型：</td>
		<td align="center" class="biaodan-q" width="18%">
          <select name="type" id='type' style="border:none; width:100%; height:100%">
              <option value="0" <c:if test="#request.type == 0">selected</c:if> >活动分类</option>
			  <option value="1" <c:if test="#request.type == 1">selected</c:if>>活动</option>
		  </select>
		</td>
  	</tr>
  	<tr>
		...
		<td align="center" class="biaodan-q" width="18%">
			<input type="text" style="border:none; width:100%; height:100%"
				   id="order" name="order" value="<s:property value="#request.name"/>" maxlength="64" >
		</td>
  	</tr>
  	<tr>
