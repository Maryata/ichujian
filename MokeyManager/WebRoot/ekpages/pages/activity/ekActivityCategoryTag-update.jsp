...
function formValid(){
	...
	if(_flag){
		if($("#addFlag").val()=="1"){
			$("#form1").attr("action","${pageContext.request.contextPath }/basedata/ekey/EKActivityCategoryTagAction!addTag.action");
		}else{
			$("#form1").attr("action","${pageContext.request.contextPath }/basedata/ekey/EKActivityCategoryTagAction!updateTag.action");
		}
	}
	return _flag;
}
...
function uploadFile(btn,fileId,baseId) {
	...
	$.ajaxFileUpload( {// ekey : 1.首页，2.活动，3.游戏
		url : "${pageContext.request.contextPath }/LogoUploadServlet.do?ekey=2&type=activityTag/logo&id="+$("#cid").val(),//用于文件上传的服务器端请求地址
