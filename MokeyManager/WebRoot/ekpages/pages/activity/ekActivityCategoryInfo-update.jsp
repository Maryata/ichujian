...
function formValid(){
	...
	if(_flag){
		if($("#addFlag").val()=="1"){
			$("#form1").attr("action","${pageContext.request.contextPath }/basedata/ekey/EKActivityCategoryInfoAction!addInfo.action");
		}else{
			$("#form1").attr("action","${pageContext.request.contextPath }/basedata/ekey/EKActivityCategoryInfoAction!updateInfo.action");
		}
	}
	return _flag;
}
...
function uploadFile(btn,fileId,baseId,vid) {
	...
	$.ajaxFileUpload( {// ekey : 1.首页，2.活动，3.游戏
		url : "${pageContext.request.contextPath }/LogoUploadServlet.do?ekey=2&type=activityCategory/"+_file+"&id="+tid,//用于文件上传的服务器端请求地址
...
<table width="40%"  border="0" align="center" cellpadding="0" cellspacing="1" class="tbg sortable">
  	...
  	<tr> 
	    ...
	    <td align="center" class="biaodan-q">
	    	<input type="text" style="border:none; width:100%; height:100%"
	    	 id="cname" name="cname" value="<s:property value="#request.cname"/>" maxlength="8" >
	    </td>
  	</tr>
  	<tr>
	...
	<tr>
		<td width="30%"  height="23" class="biaodan-top" align="left">图片：</td>
		<td align="center" class="biaodan-q">
	    	<span class="file-box">
	...
	<tr>
		<td width="30%"  height="23" class="biaodan-top" align="left">类型：</td>
		<td align="center" class="biaodan-q">
			<%--<input type="text" style="border:none; width:100%; height:100%"
				   id="type" name="type" value="<s:property value="#request.type"/>" >--%>
			<select id="type" name="type" style="border:none; width:100%; height:100%">
				<option value="1" <s:if test="#request.type ==1">selected</s:if>>分类</option>
			</select>
		</td>
	</tr>
  	<tr> 
