...
<form id="form1" method="post" action="${pageContext.request.contextPath}/basedata/ekey/eKMallAdvertAction!toAdd.action">
...
$(function(){
	// 查询应用总数
	$.post("${pageContext.request.contextPath}/basedata/ekey/eKMallAdvertAction!list.action",
		function(data){
			...
			if(count != 0){
				...
				// 分页显示广告位
				getCatePage("${pageContext.request.contextPath}/basedata/ekey/eKMallAdvertAction!list.action");
			}
	});
	// 查询广告位(第一页)
	$.post("${pageContext.request.contextPath}/basedata/ekey/eKMallAdvertAction!list.action",
		function(data){
			...
			$(list).each(function(){
				...
				_s += "<td align='center'><input type='button' value='修 改' class='butt' onClick='modifyCate("+this.C_ID+",\""+this.C_NAME+"\",\""+this.C_PID+"\",\""+this.C_ORDER+"\",\""+this.C_IMG+"\")'/>";
...
function deleteCate(cid){
    ...
    if(flag){
		$.post("${pageContext.request.contextPath}/basedata/ekey/eKMallAdvertAction!toDel.action",
...
// 修改
function modifyCate(cid,cname,pid,order,logo){
	$("#form2").attr("action","${pageContext.request.contextPath}/basedata/ekey/eKMallAdvertAction!toUpdate.action");
	$("#editId").val(cid);
	$("#editName").val(cname);
	$("#editLogo").val(logo);
	$("#editPid").val(pid);
