UPDATE_TAGS="";
Ext.holdtypeInfo = function() {
	return {
		priData : {
			categoryData : [{key:'0',value:'否'},{key:'1',value:'是'}]
		},
		formId : 'holdtypeInfo_formId',
		gridId : 'holdtypeInfo_gridId',
		addWinId : 'holdtypeInfo_addWinId',
		textwidth : 120,
		init : function(){
		...
		priSaveFun : function(){
			...
			if(dirfrom.Ext_cid ){
				saveParam.c_old_id = dirfrom.Ext_cid;
			}
			...
			dirfrom.submit({
				method : 'post',
				url : rootPath+"basedata/holdTypeInfo!saveHoldTypeInfo.action",
				waitMsg : 'save ing...',
		...
		priCreateWinFun : function(){
			...
			function checkExits(name,value,id) {
				...
				conn.open("post", rootPath+"basedata/holdTypeInfo!checkHoldTypeInfo.action", false);// false表示同步调用
			...
			function checkExitsID(id) {
				var conn = Ext.lib.Ajax.getConnectionObject().conn;
				conn.open("post", rootPath+"basedata/holdTypeInfo!checkHoldTypeID.action", false);// false表示同步调用
				conn.setRequestHeader("Content-Type","application/x-www-form-urlencoded;charset=UTF-8");
				conn.send("id="+id);
				var data = Ext.util.JSON.decode(conn.responseText);
				return data;
		...
		createGrid : function(){
    		...
    		var resultsGrid_store = new Ext.data.Store({ 
    			proxy : new Ext.data.HttpProxy({
    				url : rootPath+"basedata/holdTypeInfo!getHoldTypeInfoList.action",
    				timeout : 60000
    			}),
    		...
    		var tempArray = [
               { header: "ID", sortable: true, dataIndex: 'C_ID',width:15},
               { header: "类型名称", sortable: true, dataIndex: 'C_NAME',width:60},
    		  ...
    		  if (btn) {
    			...
    			if(record&&control=='delete'){
    				if(window.confirm('是否确认删除？')){
						...
						loadMask.show();
						Ext.Ajax.request({
				            url : rootPath+"basedata/holdTypeInfo!deleteHoldTypeInfo.action",
				            timeout : 120000,
