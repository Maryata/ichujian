Ext.actionEareInfo = function() {
	return {
		...
		formId : 'actionEareInfo_formId',
		gridId : 'actionEareInfo_gridId',
		addWinId : 'actionEareInfo_addWinId',
		textwidth : 120,
		init : function(){
		...
		priCreateWinFun : function(){
			...
			var type = _this.createComVarField({fieldLabel:"类型",id: _this.addWinId + 'c_type',name:'c_type' , data:_this.priData.typeData , width: 140,isNoBlank:true});
		...
		createGrid : function(){
    		...
    		var resultsGrid_store = new Ext.data.Store({ 
    			proxy : new Ext.data.HttpProxy({
    				url : rootPath+"basedata/actionEareInfo!getActionEareInfoList.action",
    				timeout : 60000
    			}),
    		...
    		var tempArray = [
               ...
               { header: "地域名称", sortable: true, dataIndex: 'C_NAME',width:60},
               { header: "编号", sortable: true, dataIndex: 'C_NUMBER',width:40},
               { header: "类型", sortable: true, dataIndex: 'C_TYPE',width:40,
            	   renderer: function (value, meta, record) {
            		   return value==0?'其他':'热门';
            	   }
            	   },
    		  ...
    		  if (btn) {
    			...
    			if(record&&control=='delete'){
    				if(window.confirm('是否确认删除？')){
						...
						loadMask.show();
						Ext.Ajax.request({
				            url : rootPath+"basedata/actionEareInfo!deleteActionEareInfo.action",
				            timeout : 120000,
    				...
    				dirfrom.findField('c_number').setValue(data.C_NUMBER);
