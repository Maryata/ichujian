...
Ext.actionCanalDicInfo = function() {
	return {
		//定义下拉框
		priData : {
			categoryData : [{key:'1',value:'是'},{key:'0',value:'否'}]
		},
		formId : 'actionCanalDicInfo_formId',
		gridId : 'actionCanalDicInfo_gridId',
		addWinId : 'actionCanalDicInfo_addWinId',
		textwidth : 120,
		init : function(){
		...
		priSaveFun : function(){
			...
			saveParam.c_canalid = dirfrom.findField('c_canalid').getValue();
			...
			dirfrom.submit({
				method : 'post',
				url : rootPath+"basedata/actionCanalDicInfo!saveActionCanalDicInfo.action",
				waitMsg : 'save ing...',
...
///*			function checkExits(name,value,id) {
		...
		//		conn.open("post", rootPath+"basedata/actionCanalDicInfo!checkActionCanalDicInfo.action", false); false表示同步调用				conn.setRequestHeader("Content-Type","application/x-www-form-urlencoded;charset=UTF-8");
		...
		createGrid : function(){
    		...
    		var resultsGrid_store = new Ext.data.Store({ 
    			proxy : new Ext.data.HttpProxy({
    				url : rootPath+"basedata/actionCanalDicInfo!getActionCanalDicInfoList",
    				timeout : 60000
    			}),
    		...
    		var tempArray = [
               ...
               { header: "表名", sortable: true, dataIndex: 'C_NAME',width:60},
               { header: "渠道号", sortable: true, dataIndex: 'C_CANALID',width:60},
    		  ...
    		  if (btn) {
    			...
    			if(record&&control=='delete'){
    				if(window.confirm('是否确认删除？')){
						...
						loadMask.show();
						Ext.Ajax.request({
				            url : rootPath+"basedata/actionCanalDicInfo!deleteActionCanalDicInfo.action",
				            timeout : 120000,
    				...
    				dirfrom.findField('c_canalid').setValue(data.C_CANALID);
    				//dirfrom.findField('c_id').setReadOnly(true);	
    			}
    		  }   
    		},   
