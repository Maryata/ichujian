...
Ext.industryType = function() {
	return {
		...
		priSaveFun : function(){
			...
			saveParam.c_level = dirfrom.findField('c_level').getValue();
			...
			dirfrom.submit({
				method : 'post',
				url : rootPath+"basedata/actionIndustryType!saveActionInfo.action",
				waitMsg : 'save ing...',
		...
		createGrid : function(){
    		...
    		var resultsGrid_store = new Ext.data.Store({ 
    			proxy : new Ext.data.HttpProxy({
    				url : rootPath+"basedata/actionIndustryType!getActionTypeList.action",
    				timeout : 60000
    			}),
    			reader : new Ext.data.JsonReader({
    				totalProperty : 'count',
    		...
    		var tempArray = [
    			{ header: "父节点", sortable: true, dataIndex: 'C_PARID',width:40,
    				renderer:function (value, meta, record){
            	   		if(!Ext.isEmpty(record.get('P_NAME'))){
            	   			return record.get('P_NAME');
            	   		}
            	        return value==0?'总类型':value;
            	   }
    			
    			},
               { header: "ID", hidden: true, dataIndex: 'C_ID',width:15},
               { header: "行业类别名称", sortable: true, dataIndex: 'C_NAME',width:60},
               { header: "级别", sortable: true, dataIndex: 'C_LEVEL',width:40},
    		  ...
    		  if (btn) {
    			...
    			if(record&&control=='delete'){
    				if(window.confirm('是否确认删除？')){
						...
						loadMask.show();
						Ext.Ajax.request({
				            url : rootPath+"basedata/actionIndustryType!deleteActionTypeInfo.action",
				            timeout : 120000,
