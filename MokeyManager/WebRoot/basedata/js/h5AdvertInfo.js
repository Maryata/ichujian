Ext.h5AdvertInfo = function() {
	return {
		priData : {
		},
		formId : 'h5AdvertInfo_formId',
		gridId : 'h5AdvertInfo_gridId',
		addWinId : 'h5AdvertInfo_addWinId',
		textwidth : 120,
		init : function(){
		...
		load : function(){
			...
			// 游戏APP的下拉的菜单
			_this.getAppStore = function(){
				var AppStore = new Ext.data.Store( {
					proxy : new Ext.data.HttpProxy( {
						url : rootPath + "basedata/h5GameAction!ajaxList.action?limit=999",
						timeout : 60000 
					}),
			...
			_this.AppStore = _this.getAppStore(); 
		...
		priSaveFun : function(){
			...
			dirfrom.submit({
				method : 'post',
				url : rootPath+"basedata/h5AdvertInfo!saveAdvertIfo.action",
				waitMsg : 'save ing...',
		...
		priCreateWinFun : function(){
			...
			function checkExits(appType,checkType, value,idVal) {
				...
				conn.open("post", rootPath+"basedata/h5AdvertInfo!checkAdvertIfo.action", false);// false表示同步调用
...
//                {name : 'C_TYPE'},
    		...
    		var resultsGrid_store = new Ext.data.Store({ 
    			proxy : new Ext.data.HttpProxy({
    				url : rootPath+"basedata/h5AdvertInfo!getAdvertIfoList.action",
    				timeout : 60000
    			}),
    			reader : new Ext.data.JsonReader({
    				totalProperty : 'count',
    		...
    		var tempArray = [
               { header: "C_ID", hidden: true, dataIndex: 'C_ID'},
    		  ...
    		  if (btn) {
    			...
    			if(record&&control=='delete'){
    				if(window.confirm('是否确认删除？')){
						...
						loadMask.show();
						Ext.Ajax.request({
				            url : rootPath+"basedata/h5AdvertInfo!deleteAdvertIfo.action",
				            timeout : 120000,
...
//    				dirfrom.findField('c_appid').store.load({
//    					params :{ c_appid : data.C_APPID  },
//    					callback: function(obj){
//    						dirfrom.findField('c_appid').setValue(data.C_APPID);
//    						dirfrom.findField('c_appid').setRawValue(data.C_APPNAME);
//    					}
