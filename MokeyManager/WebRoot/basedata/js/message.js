(function(global, $) {
	...
	var list = function(queryParam) {
		...
		$.ajax({
			type : 'GET',
			url : basePath + '/basedata/messageAction!ajaxList.action',
			data : queryParam,
			dataType : 'json',
			success : function( data ) {
				if( data && data.status === 'Y') {
					...
					for(var i = 0; i < l.length; ++i) {
						...
						tr += '<td nowrap="nowrap">'+replied(o[cols[j]])+'</td>'
	...
	Message.toEdit = function(id) {
		global.location.href = basePath + '/basedata/messageAction!toEdit.action?uid=' + id;
	};
	...
	Message.del = function(id) {
		if(id) {
			if(confirm('确定删除本条数据吗？')) {
				$.ajax({
					type : 'POST',
					url : basePath + '/basedata/messageAction!ajaxDelete.action',
					data : {'c_id' : id},
					dataType : 'json',
					success : function( data ) {
						if( data && data.status === 'Y') {
