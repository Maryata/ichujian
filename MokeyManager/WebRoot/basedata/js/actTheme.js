...
Ext.industryType = function() {
	return {
		//定义下拉框
		priData : {
			categoryData : [{key:'1',value:'是'},{key:'0',value:'否'}]
		},
		formId : 'industryType_formId',
		gridId : 'industryType_gridId',
		addWinId : 'industryType_addWinId',
		textwidth : 120,
		init : function(){
		...
		createComVarField : function(option){
			...
			if(option.anchor){
				combObj.anchor = option.anchor;
			}
			return  new Ext.form.ComboBox(combObj);
		},
		//***************************
		...
		priSaveFun : function(){
			...
			saveParam.c_istitle = dirfrom.findField('c_istitle').getValue(); 
		    saveParam.c_order = dirfrom.findField('c_order').getValue();
//		    saveParam.c_parid = dirfrom.findField('c_parid').getValue();
		...
		priCreateWinFun : function(){
			...
			var typeField2 = _this.createComVarField({fieldLabel:"是否为标题",id: _this.addWinId + 'c_istitle',name:'c_istitle' , data:_this.priData.categoryData , anchor : '98%',isNoBlank:true});
			
			var functionNameStore = new Ext.data.Store({
            	//autoLoad: true,
            	proxy : new Ext.data.HttpProxy({
            		url : rootPath+"basedata/actionIndustryType!findAllName.action",
            	    timeout : 60000
            	}),
			...
			function checkExits(name,value,id) {
				...
				conn.open("post", rootPath+"basedata/actionIndustryType!checkActionTypeInfo.action", false);// false表示同步调用
		...
		createGrid : function(){
    		...
    		var resultsGrid_store = new Ext.data.Store({ 
    			proxy : new Ext.data.HttpProxy({
    				url : rootPath+"basedata/actionActTheme!getActThemeList.action",
    				timeout : 60000
    			}),
    			reader : new Ext.data.JsonReader({
    				totalProperty : 'count',
    		...
    		var tempArray = [
               ...
               { header: "活动专题名称", sortable: true, dataIndex: 'C_NAME',width:100},
               { header: "是否标题", sortable: true, dataIndex: 'C_ISTITLE',width:70,
            	   renderer: function (value, meta, record) {  
        		   return value==0?'否':'是'; 
        	   }  },
               { header: "排序", sortable: true, dataIndex: 'C_ORDER',width:70},
               { header: "是否有效", sortable: true, dataIndex: 'C_ISLIVE',width:50,
            	   renderer: function (value, meta, record) {  
            		   return value==0?'否':'是';
            	   }},
               
               { header: "操作", sortable: false, dataIndex: '',width:50,align:'center',
        	     renderer: function (value, meta, record) {
    		  ...
    		  if (btn) {
    			...
    			if(record&&control=='delete'){
    				if(window.confirm('是否确认删除？')){
						...
						loadMask.show();
						Ext.Ajax.request({
				            url : rootPath+"basedata/actionActTheme!deleteById.action",
				            timeout : 120000,
    				...
    				dirfrom.findField('c_parid').setValue(data.C_PARID);//实际值
    				if('0'==data.C_PARID){
    					dirfrom.findField('c_parid').setRawValue('总类型'); 
    				}
    				    dirfrom.findField('c_parid').setRawValue(data.P_NAME);  //显示值
