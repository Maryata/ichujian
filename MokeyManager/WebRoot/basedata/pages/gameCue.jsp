...
function deleteCol(cid){
    ...
    if(flag){
		$.post("${pageContext.request.contextPath}/basedata/gameCueAction!deleteCue.action",
...
function modifyCol(cid,ctitle){ 
	var answer=window.open("${pageContext.request.contextPath}/basedata/pages/gameCue-modify.jsp?cid="+cid+"&ctitle="+ctitle,
...
function isvalid(cid,islive){
	$.post("${pageContext.request.contextPath}/basedata/gameCueAction!isvalid.action",
...
function handle(cid,cname){
	var answer=window.open("${pageContext.request.contextPath}/basedata/pages/gameCol-handle.jsp?cid="+cid+"&cname="+cname,
			"游戏合集分类","height=550, width=650, top=100, left=450, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no"); 
}
// 校验新增合集名
function checkAdd(){
	...
	$.post("${pageContext.request.contextPath}/basedata/gameCueAction!addCue.action",
