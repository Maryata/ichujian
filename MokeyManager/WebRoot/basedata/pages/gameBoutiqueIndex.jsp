...
<table width="98%" height="35"  border="0" align="center" cellpadding="1" >
  <tr>
    <td width="62.7%" class="tz_crumbs"><img src="${pageContext.request.contextPath}/images/crumbs.jpg" width="9" height="9" /> 系统信息 -&gt; 3号键管理 -&gt; 精品游戏首页排版维护 </td>
  </tr>
</table>
...
function cateList(appName){
	...
	$.post("${pageContext.request.contextPath}/basedata/gameBoutiqueIndexAction!cateList.action",{name:appName},
		function(data){
			...
			if(count != 0){
				...
				// 分页查询所有分类
				getAppPage("${pageContext.request.contextPath}/basedata/gameBoutiqueIndexAction!cateList.action",appName);
			}
	});
	// 查询分类(第一页)
	$.post("${pageContext.request.contextPath}/basedata/gameBoutiqueIndexAction!cateList.action",{name:appName},
...
function indexCateList(cateName){
	...
	$.post("${pageContext.request.contextPath}/basedata/gameBoutiqueIndexAction!indexCateList.action",{name:cateName},
		function(data){
			var json = $.parseJSON(data);
			var count = json[0].count;// 数据总数
			$("#page22").empty();
			if(count != 0){
				...
				// 当前首页显示的分类
				getIndexCateList("${pageContext.request.contextPath}/basedata/gameBoutiqueIndexAction!indexCateList.action",cateName);
			}
	});
	// 查询首页显示的分类(第一页)
	$.post("${pageContext.request.contextPath}/basedata/gameBoutiqueIndexAction!indexCateList.action",{name:cateName},
...
function submitt(){
	...
	if(flag){
		$.post("${pageContext.request.contextPath}/basedata/gameBoutiqueIndexAction!handle.action",{id:id,removeId:removeId,order:order,number:number},function(data){
