...
<table width="98%" height="35"  border="0" align="center" cellpadding="1" >
  <tr>
    <td align="left" class="jg_fc">当前H5游戏分类&nbsp;-&gt;&nbsp;<s:property value="#request.cname"/>&nbsp;&nbsp;&nbsp;</td>
  </tr>
</table>
  ...
  <tr>
    <td>
      <table width="100%"  border="0" cellspacing="1" cellpadding="2">
        <tr> 
         <td width="12.1%" height="23" class="biaodan-top" align="left">H5游戏名称：</td>
      ...
      <table width="100%"  border="0" cellspacing="1" cellpadding="2">
        <tr> 
         ...
         <td width="61%" class="biaodan-q" align="left">
			<input type="text" name="recGameName" style="border:none; width:100%; height:100%" id="recGameName" />
		 </td>
         <td align="center" class="biaodan-q">
           <input name="Button" type="button" class="butt" value="查 询" onclick="recommendGameList()" />
           </td>
        </tr>
      </table>
    </td>
...
<input type="hidden" id="recomendTotal" />
<div id="page22"></div>
</form>

<script>
// 全局数组，用来存放移除出当前推荐的H5游戏id
var removeGid = new Array();
// 定义Map，存放移除推荐的H5游戏的排序
var orderMap = {};
...
function gameList(){
	...
	// 查询未作为推荐的H5游戏
	$.post("${pageContext.request.contextPath}/basedata/h5GameRecommendAction!getAllGame.action",{name:name,gameCate:gameCate},
		function(data){
			var json = eval("("+data+")");
			...
			if(json != 0){
				...
				// 所有H5游戏
				getGamePage("${pageContext.request.contextPath}/basedata/h5GameRecommendAction!getAllGame.action",name,gameCate);
			}
	});
	$.post("${pageContext.request.contextPath}/basedata/h5GameRecommendAction!getAllGame.action",{name:name,gameCate:gameCate},
		function(data){
...
// 当前推荐的所有H5游戏
function recommendGameList(){
	var name = $("#recGameName").val();
	// 查询当前推荐的H5游戏总数
	$.post("${pageContext.request.contextPath}/basedata/h5GameRecommendAction!recommendGameList.action",{name:name},
		function(data){
			var json = eval("("+data+")");
			var count = json[0].count;// 数据总数
			$("#page22").empty();
			if(json != 0){
				setTotal(count,"recomendTotal");
				// 当前推荐的H5游戏
				currRecGamePage("${pageContext.request.contextPath}/basedata/h5GameRecommendAction!recommendGameList.action",name);
			}
	});
	$.post("${pageContext.request.contextPath}/basedata/h5GameRecommendAction!recommendGameList.action",{name:name},
		function(data){
...
// 分页查询所有H5游戏
function getGamePage(url,name,gameCate) {
	laypage({
	    cont: 'page11',
	    ...
	    jump: function(e, first){ //触发分页后的回调
	        if(!first){ //一定要加此判断，否则初始时会无限刷新
	           $.post(url,{page:e.curr,name:name,gameCate:gameCate},
					function(data){
						json = eval("("+data+")");
						var list = json[0].list;
						$("#table1").empty();
...
// 分页查询当前推荐的H5游戏
function currRecGamePage(url,name) {
	laypage({
	    cont: 'page22',
	    pages: document.getElementById("recomendTotal").value,
	    curr: function(){ //通过url获取当前页，也可以同上（pages）方式获取
	        var page = location.search.match(/page=(\d+)/);
	        return page ? page[1] : 1;
	    }(), 
...
function up(){
	$("#table2 input:checked").each(function(){
	  var tablerow = $(this).parent("td").parent("tr");
	  var id = tablerow.find("input[name='GID']").val();
	  // 将要移除出当前分类的H5游戏id存入全局数组
	  removeGid.push(id);
	  // 将要移除出当前分类的H5游戏的order存入全局Map
...
function submitt(){
	...
	$.post("${pageContext.request.contextPath}/basedata/h5GameRecommendAction!handle.action",{gid:gid,removeGid:removeGid,order:order},function(data){
