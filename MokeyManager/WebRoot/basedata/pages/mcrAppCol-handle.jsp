...
<table width="98%" height="35"  border="0" align="center" cellpadding="1" >
  <tr>
    <td align="left" class="jg_fc">当前应用合集&nbsp;-&gt;&nbsp;<s:property value="#request.name"/>&nbsp;&nbsp;&nbsp;</td>
  </tr>
</table>
    ...
    <td>
      <table width="100%"  border="0" cellspacing="1" cellpadding="2">
        <tr> 
         ...
         <td align="center" class="biaodan-q">
           <input name="Button" type="button" class="butt" value="查 询" onclick="getColAppCondition()" />
           </td>
        </tr>
      </table>
    </td>
...
$(function(){
	var cid = $("#cid").val();
	// 查询应用总数
	$.post("${pageContext.request.contextPath}/basedata/mcrAppColAction!getAppTotal.action",{cid:cid},
		function(data){
			var json = eval("("+data+")");
			$("#page11").empty();
			if(json != 0){
				$("#appTotal").val(json);
				// 所有应用
				getAppPage("${pageContext.request.contextPath}/basedata/mcrAppColAction!getAppList.action",cid);
			}
	});
	// 查询当前合集的应用总数
	$.post("${pageContext.request.contextPath}/basedata/mcrAppColAction!getColTotal.action",{cid:cid},
		function(data){
			var json = eval("("+data+")");
			$("#page22").empty();
			if(json != 0){
				$("#colTotal").val(json);
				// 当前合集中的应用
				getGamePageByColId("${pageContext.request.contextPath}/basedata/mcrAppColAction!getColList.action",cid);
			}
	});
	$.post("${pageContext.request.contextPath}/basedata/mcrAppColAction!getAppList.action",{cid:cid},
		function(data){
			json = eval("("+data+")");
			$(json).each(function(){
	...
	$.post("${pageContext.request.contextPath}/basedata/mcrAppColAction!getColList.action",{cid:cid},
		function(data){
			json = eval("("+data+")");
			$(json).each(function(){
				$("#table2").append("<tr class='biaodan-q'><input type='hidden' name='AID' value="+this.C_ID+" /><td align='center'><input type='checkbox' name='ckbox1'></td><td align='center' name='ANAME'>"+this.C_NAME+"</td><td align='center'><input name='CORDER' value="+(this.C_ORDER||'')+" ></input></td></tr>");
			});
	});
});
// 分页查询所有应用
...
function getAppCondition(){
	...
	// 条件查询应用总数
	$.post("${pageContext.request.contextPath}/basedata/mcrAppColAction!getTotalCondition.action",{cid:cid,name:name},
		function(data){
			var json = eval("("+data+")");
			$("#page11").empty();
			if(json != 0){
				$("#appTotal").val(json);
				// 分页查询
				conditionPage("${pageContext.request.contextPath}/basedata/mcrAppColAction!getListCondition.action",cid,name);
			}
	});
	$.post("${pageContext.request.contextPath}/basedata/mcrAppColAction!getListCondition.action",{cid:cid,name:name},
		function(data){
			json = eval("("+data+")");
			$("#table1").empty();
			$(json).each(function(){
...
function conditionPage(url,cid,name){
	laypage({
	    cont: 'page11',
	    pages: document.getElementById("appTotal").value,
	    curr: function(){ //通过url获取当前页，也可以同上（pages）方式获取
...
// 条件查询合集中的应用
function getColAppCondition(){
	var cid = $("#cid").val();
	var name = $("#colAppName").val();
	// 条件查询合集应用总数
	$.post("${pageContext.request.contextPath}/basedata/mcrAppColAction!getColTotalCondition.action",{cid:cid,name:name},
		function(data){
			var json = eval("("+data+")");
			$("#page22").empty();
			if(json != 0){
				$("#colTotal").val(json);
				// 分页查询
				colConditionPage("${pageContext.request.contextPath}/basedata/mcrAppColAction!getColListCondition.action",cid,name);
			}
	});
	$.post("${pageContext.request.contextPath}/basedata/mcrAppColAction!getColListCondition.action",{cid:cid,name:name},
		function(data){
			json = eval("("+data+")");
			$("#table2").empty();
			$(json).each(function(){
...
function up(){
	$("#table2 input:checked").each(function(){
	  var tablerow = $(this).parent("td").parent("tr");
	  var id = tablerow.find("input[name='AID']").val();
	  // 将要移除出当前合集的应用id存入全局数组
	  removeAid.push(id);
	  // 将要移除出当前合集的应用的合集order存入全局Map
...
function submitt(){
	...
	$.post("${pageContext.request.contextPath}/basedata/mcrAppColAction!handleCol.action",{cid:cid,aid:aid,removeAid:removeAid,order:order},function(data){
