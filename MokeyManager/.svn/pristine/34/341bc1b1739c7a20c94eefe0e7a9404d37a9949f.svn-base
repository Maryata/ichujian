...
function formValid(){
	...
	if(_flag){
		if($("#addFlag").val()=="1"){
			$("#form1").attr("action","${pageContext.request.contextPath }/basedata/ekey/eKIndexAdvertAction!addAdvert.action");
		}else{
			$("#form1").attr("action","${pageContext.request.contextPath }/basedata/ekey/eKIndexAdvertAction!updateAdvert.action");
		}
	}
	return _flag;
}
...
<table width="80%"  border="0" align="center" cellpadding="0" cellspacing="1" class="tbg sortable">
	...
	<br/>
  	<tr>
		...
		<td width="30%" align="center" class="biaodan-q">
			<select  name="aid" id="aid"  style="width:100%;border:none;hight:100%;" m="search">
				...
				<c:forEach var="item" items="${list}" varStatus="status">
					<option data-val="${item.C_IMG}" value="${item.C_ID}" <c:if test="${item.C_ID==aid}">selected</c:if>>${item.C_NAME}</option>
				</c:forEach>
			</select>
		</td>
		...
		<%--<td width="8%"  height="23" class="biaodan-top" align="left">app查询：</td>
		<td width="10%" align="center" class="biaodan-q">
			<input type="text" style="border:none; width:100%; height:100%" id="tname" name="tname"/>
		</td>
		<td width="10%" align="center" class="biaodan-q">
			<input type="button" style="border:none; width:100%; height:100%" id="btn" onclick="btnclick()" value="查询"/>
		</td>--%>
  	</tr>
	...
	<script>
		/*function  btnclick(){
		var name = $("#tname").val();
		$.post("${pageContext.request.contextPath}/basedata/ekey/eKIndexAdvertAction!toSelect.action",
				{name:name},function(data){
					var item = $.parseJSON(data);
					$("#aid").empty();
					$(item).each(function(){
						var _tr="<option data-val='"+this.C_IMG+"' value='"+this.C_ID+"'>"+this.C_NAME+"</option>";
						$("#aid").append(_tr);
					})
				});
		}*/
	...
	<tr>
		<td width="8%"  height="23" class="biaodan-top" align="left">广告位置：</td>
		<td width="30%" align="center" class="biaodan-q" colspan="2">
			<select  name="type" id="type"  style="width:100%;border:none;hight:100%;">
				<option value="1" <c:if test="${type==1}">selected</c:if>>顶部广告位</option>
				<option value="2" <c:if test="${type==2}">selected</c:if>>中部广告位</option>
			</select>
		</td>
	</tr>
  	<tr>
