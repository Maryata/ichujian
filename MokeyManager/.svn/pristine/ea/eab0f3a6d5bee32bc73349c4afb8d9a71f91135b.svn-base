...
function gameList(){
	...
	// 查询未作为推荐的游戏
	$.post("${pageContext.request.contextPath}/basedata/gameSearchAction!getAllGame.action",{name:name,gameCate:gameCate},
		function(data){
			var json = eval("("+data+")");
			...
			if(json != 0){
				...
				// 所有游戏
				getGamePage("${pageContext.request.contextPath}/basedata/gameSearchAction!getAllGame.action",name,gameCate);
			}
	});
	$.post("${pageContext.request.contextPath}/basedata/gameSearchAction!getAllGame.action",{name:name,gameCate:gameCate},
		function(data){
...
function recommendGameList(){
	...
	// 查询当前推荐的游戏总数
	$.post("${pageContext.request.contextPath}/basedata/gameSearchAction!recommendGameList.action",{name:name},
		function(data){
			var json = eval("("+data+")");
			var count = json[0].count;// 数据总数
			$("#page22").empty();
			if(json != 0){
				...
				// 当前推荐的游戏
				currRecGamePage("${pageContext.request.contextPath}/basedata/gameSearchAction!recommendGameList.action",name);
			}
	});
	$.post("${pageContext.request.contextPath}/basedata/gameSearchAction!recommendGameList.action",{name:name},
		function(data){
...
function submitt(){
	...
	$.post("${pageContext.request.contextPath}/basedata/gameSearchAction!handle.action",{gid:gid,removeGid:removeGid,order:order},function(data){
