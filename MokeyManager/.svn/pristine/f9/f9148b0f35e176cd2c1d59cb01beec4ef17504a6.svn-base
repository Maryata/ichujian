 ...
 * 游戏帮：精品游戏首页排版
 */
public class GameBoutiqueIndexDaoImpl implements GameBoutiqueIndexDao {
	
	private static final Logger LOGGER = Logger.getLogger(GameBoutiqueIndexDaoImpl.class);
	...
	public Map<String, Object> indexCateList(String name, int start, int limit) {
		...
		try {
			StringBuffer sql = new StringBuffer("SELECT COUNT(1) FROM" +
					...
					" AND T.C_TYPE = 1 AND C.C_TYPE = '02' AND T.C_CID = C.C_ID"); 
			...
			LOGGER.error("GameBoutiqueIndexDaoImpl.indexCateList failed, e : " + e);
		}
		return ret;
	}
	...
	public Integer isExist(String cid) {
		try {
			StringBuffer sql = new StringBuffer("SELECT COUNT(*) FROM" +
					" T_GAME_CUSTOM T WHERE T.C_TYPE = 1 AND T.C_CID = ?");
			List<Object> args = new ArrayList<Object>();
			args.add(cid);
			return this.jdbcTemplate.queryForObject(sql.toString(), args.toArray(), Integer.class);
		} catch (Exception e) {
			LOGGER.error("GameBoutiqueIndexDaoImpl.isExist failed, e : " + e);
		}
		return null;
	}
	...
	public void update(String cid, String number, String order) {
		try {
			...
			LOGGER.error("GameBoutiqueIndexDaoImpl.update failed, e : " + e);
		}
	}
	...
	public void add(String cid, String number, String order) {
		try {
			StringBuffer sql = new StringBuffer("INSERT INTO" +
					...
					" VALUES (SEQ_GAME_CUSTOM.NEXTVAL, ?, 1, ?, ?)");
			this.jdbcTemplate.update(sql.toString(), new Object[]{cid,number,order});
		} catch (Exception e) {
			LOGGER.error("GameBoutiqueIndexDaoImpl.add failed, e : " + e);
		}
	}
	...
	public void remove(String cid) {
		try {
			...
			LOGGER.error("GameBoutiqueIndexDaoImpl.remove failed, e : " + e);
		}
	}
	...
	public Map<String, Object> cateList(int page, String name) {
		...
		try {
			StringBuffer sql = new StringBuffer("SELECT COUNT(1)" +
					" FROM T_GAME_CATEGORY C WHERE C.C_ISLIVE = 1 AND C.C_TYPE = '02'"); 
			List<Object> argsList = new ArrayList<Object>();
			if (StrUtils.isNotEmpty(name)) {
				sql.append(" AND C.C_NAME LIKE ?");
				argsList.add("%" + name + "%");
			...
			sql.append(" AND NOT EXISTS(SELECT T.C_CID FROM" +
					" T_GAME_CUSTOM T WHERE T.C_TYPE = 1 AND T.C_CID = C.C_ID)");
			int count = jdbcTemplate.queryForObject(sql.toString(), argsList.toArray(), Integer.class);
			...
			LOGGER.error("GameBoutiqueIndexDaoImpl.cateList failed, e : " + e);
		}
		return ret;
	}
