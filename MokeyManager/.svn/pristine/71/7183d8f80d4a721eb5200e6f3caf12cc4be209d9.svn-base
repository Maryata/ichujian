...
<table width="98%" height="35"  border="0" align="center" cellpadding="1" >
  <tr>
    <td width="62.7%" class="tz_crumbs"><img src="${pageContext.request.contextPath}/images/crumbs.jpg" width="9" height="9" /> 系统信息 -&gt; 3号键管理 -&gt; H5精品游戏首页排版维护 </td>
  </tr>
</table>
    ...
    <td>
      <table width="100%"  border="0" cellspacing="1" cellpadding="2">
        <tr> 
         ...
         <td align="center" class="biaodan-q">
           <input name="Button" type="button" class="butt" value="查 询" onclick="indexCateList()" />
           </td>
        </tr>
      </table>
    </td>
...
// 定义Map，存放移除出首页的分类的排序
var orderMap = {};
// 定义Map，存放移除出首页的分类的游戏的数量
var numberMap = {};

$(function(){
	var appName = $("#appName").val();// 游戏名称
	var cateName = $("#cateName").val();// 分类中游戏的名称
	cateList(appName);// 查询游戏
	indexCateList(cateName);// 查询首页显示的分类
});
// 查询所有分类
function cateList(appName){
	...
	$.post("${pageContext.request.contextPath}/basedata/h5GameBoutiqueIndexAction!cateList.action",{name:appName},
		function(data){
			...
			if(count != 0){
				...
				// 分页查询所有分类
				getAppPage("${pageContext.request.contextPath}/basedata/h5GameBoutiqueIndexAction!cateList.action",appName);
			}
	});
	// 查询分类(第一页)
	$.post("${pageContext.request.contextPath}/basedata/h5GameBoutiqueIndexAction!cateList.action",{name:appName},
...
// 查询首页显示的分类
function indexCateList(cateName){
	if(!cateName){
		cateName = $("#cateName").val();
	}
	$.post("${pageContext.request.contextPath}/basedata/h5GameBoutiqueIndexAction!indexCateList.action",{name:cateName},
		function(data){
			var json = $.parseJSON(data);
			var count = json[0].count;// 数据总数
			$("#page22").empty();
			if(count != 0){
				...
				// 当前首页显示的分类
				getIndexCateList("${pageContext.request.contextPath}/basedata/h5GameBoutiqueIndexAction!indexCateList.action",cateName);
			}
	});
	// 查询首页显示的分类(第一页)
	$.post("${pageContext.request.contextPath}/basedata/h5GameBoutiqueIndexAction!indexCateList.action",{name:cateName},
...
function submitt(){
	...
	var number = new Array();// 每个分类的游戏的数量
	// 获取下面表的所有游戏id
	...
	if(flag){
		$.post("${pageContext.request.contextPath}/basedata/h5GameBoutiqueIndexAction!handle.action",{id:id,removeId:removeId,order:order,number:number},function(data){
