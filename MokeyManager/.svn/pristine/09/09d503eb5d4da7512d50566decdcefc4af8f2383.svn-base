 ...
 * 游戏帮：游戏礼包
 */
public class GameGiftDaoImpl implements GameGiftDao {

private static final Logger LOGGER = Logger.getLogger(GameGiftDaoImpl.class);
		...
		try {
			if(StrUtils.isNotEmpty(page)){
				StringBuffer sb = new StringBuffer("SELECT T.C_ID ID,T.C_NAME CNAME,G.C_NAME GNAME," +
						...
						"T.C_COUNT COUNT FROM T_GAME_GIFTS_INFO T,T_GAME_APP_INFO G " +
			...
			LOGGER.error("gameGiftDaoImpl.gameGiftList failed, e : " + e);
		}
		return null;
	}
	...
	public Integer getTotal(String gid, String name, String gName) {
		try {
			StringBuffer sb = new StringBuffer("SELECT COUNT(*) FROM " +
					"T_GAME_GIFTS_INFO T, T_GAME_APP_INFO G WHERE T.C_ISLIVE = 1 " +
			...
			LOGGER.error("gameGiftDaoImpl.getTotalCol failed, e : " + e);
		}
		return null;
	}
	...
	public void deleteGameGiftCode(String id) {
		try {
			String sql = "DELETE FROM T_GAME_GIFTS_CODE_INFO WHERE C_GID = ?";
			this.jdbcTemplate.update(sql, new Object[]{id});
		} catch (Exception e) {
			LOGGER.error("gameGiftDaoImpl.deleteGameGiftCode failed, e : " + e);
		}
	}
	...
	public void deleteGameGift(String id) {
		try {
			String sql = "DELETE FROM T_GAME_GIFTS_INFO WHERE C_ID = ?";
			this.jdbcTemplate.update(sql,new Object[]{id});
		} catch (Exception e) {
			LOGGER.error("gameGiftDaoImpl.getTotalCol failed, e : " + e);
		}
	...
	public List<Map<String, Object>> queryGiftById(String editId) {
		try {
			String sql = "SELECT T.C_ID ID,T.C_GID GID,T.C_NAME NAME,T.C_DEPICT DEPICT," +
					...
					", T.C_TYPE CATE FROM T_GAME_GIFTS_INFO T WHERE T.C_ISLIVE = 1 AND T.C_ID = ?";
	...
	public void saveGameGift(List<Object> args) {
		try {
			String sql = "INSERT INTO T_GAME_GIFTS_INFO " +
			...
			LOGGER.error("gameGiftDaoImpl.saveGameGift failed, e : " + e);
		}
	}
	...
	public void updateGameGift(List<Object> args) {
		try {
			String sql = "UPDATE T_GAME_GIFTS_INFO T SET T.C_GID = ?, T.C_NAME = ?, " +
					"T.C_DEPICT = ?, T.C_SDATE = ?, T.C_EDATE = ?, T.C_METHOD = ?, " +
					"T.C_COUNT = ?, T.C_ORDER = ?, T.C_TYPE = ? WHERE T.C_ID = ?";
			this.jdbcTemplate.update(sql,args.toArray());
		} catch (Exception e) {
			LOGGER.error("gameGiftDaoImpl.updateGameGift failed, e : " + e);
		}
	}
	...
	public Integer scanedNum(String newId, String gid) {
		try {
			String sql = "SELECT COUNT(*) FROM T_GAME_CHILD_ACTION T WHERE T.C_TYPE = 0 " +
			...
			LOGGER.error("gameGiftDaoImpl.scanedNum failed, e : " + e);
		}
		return null;
	}
	...
	public boolean existId(String id) {
		try {
			String sql = "SELECT COUNT(*) FROM T_GAME_GIFTS_INFO T WHERE T.C_ID = ? ";
...
//				LOGGER.info("into gameGiftDaoImpl.scanedNum");
//				LOGGER.info("Counts of codes is : " + list.size());
				String sql = "INSERT INTO T_GAME_GIFTS_CODE_INFO (C_ID, C_GID, C_CODE, C_ISLIVE, C_STATE)" +
						"VALUES(?, ?, ?, ?, ?)";
	...
	public Map<String, Object> getCodesListByGid(int page, String gid,
		...
		try {
			StringBuffer sql = new StringBuffer("SELECT COUNT(1)" +
					...
					" T.C_EDATE, T.C_STATE FROM T_GAME_GIFTS_CODE_INFO T" +
			...
			sql.append(") TEMP1" +
					...
					" UNAME FROM T_GAME_MEMBER_GIFTS MG, T_GAME_MEMBER_INFO M" +
			...
			LOGGER.error("gameGiftDaoImpl.getCodesListByGid failed, e : " + e);
		}
		return ret;
	}
	...
	public void delCode(String gid, String id) {
		try {
			String sql = "DELETE FROM T_GAME_GIFTS_CODE_INFO WHERE C_GID = ? AND C_ID = ?";
			this.jdbcTemplate.update(sql, new Object[]{gid, id});
		} catch (Exception e) {
			LOGGER.error("gameGiftDaoImpl.delCode failed, e : " + e);
		}
	}
	...
	public void batchDel(List<Object[]> list) {
		try {
			...
			LOGGER.error("gameGiftDaoImpl.batchDel failed, e : " + e);
		}
	}
	...
	public List<Map<String, Object>> getCodesByGidNState(String gid,
		...
		try {
			String sql = "SELECT TEMP1.C_CODE C_CODE, USERID, UNAME" +
					" FROM (SELECT T.C_CODE FROM T_GAME_GIFTS_CODE_INFO T" +
					" WHERE T.C_ISLIVE = 1 AND T.C_GID = ? AND T.C_STATE = ?)" +
					" TEMP1 LEFT JOIN (SELECT MG.C_CODE, M.C_ID USERID," +
					" M.C_NICKNAME UNAME FROM T_GAME_MEMBER_GIFTS MG," +
					" T_GAME_MEMBER_INFO M WHERE M.C_ISLIVE = 1 AND MG.C_TYPE = 1" +
			...
			LOGGER.error("gameGiftDaoImpl.getCodesByGidNState failed, e : " + e);
		}
		return null;
	}
