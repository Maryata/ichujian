...
Ext.ux.form.LovCombo = Ext.extend(Ext.form.ComboBox, {
	...
	onBeforeQuery : function(qe) {
		// qe.query = qe.query.replace(new
		// RegExp(RegExp.escape(this.getCheckedDisplay()) + '[ ' +
		// this.separator + ']*'), '');
	...
	onSelect : function(record, index) {
		if (this.fireEvent('beforeselect', this, record, index) !== false) {
			...
			//EIDTIOR BY JASON 注释掉这里 如果是有过滤的就不用去重新加载所有的
	...
	beforeBlur : function() {
		...
		if (this.forceSelection) {
			...
			for (var i = 0; i < texts.length; i++) {
				...
				if (rec) {
					values += (values.length > 0 ? this.separator : '')
							+ rec.data[this.valueField];
				}
			}
			// alert("是否有值"+values);
