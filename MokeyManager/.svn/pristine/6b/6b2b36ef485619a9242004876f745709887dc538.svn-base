UPDATE_TAGS="";
Ext.menuinfo = function() {
	return {
		priData : {
			categoryData : [{key:'0',value:'否'},{key:'1',value:'是'}]
		},
		formId : 'menuinfo_formId',
		gridId : 'menuinfo_gridId',
		addWinId : 'menuinfo_addWinId',
		textwidth : 120,
		init : function(){
		...
		priSaveFun : function(){
			...
			dirfrom.submit({
				method : 'post',
				url : rootPath+"basedata/menuinfo!Save.action",
				waitMsg : 'save ing...',
		...
		priCreateWinFun : function(){
            ...
            var functionNameStore = new Ext.data.Store({
            	...
            	proxy : new Ext.data.HttpProxy({
            		url : rootPath+"basedata/menuinfo!findName.action",
            	    timeout : 60000
            	}),
			...
			function checkExits(name,id) {
				...
				conn.open("post", rootPath+"basedata/menuinfo!checkName.action", false);// false表示同步调用
		...
		createGrid : function(){
    		...
    		var resultsGrid_store = new Ext.data.Store({ 
    			proxy : new Ext.data.HttpProxy({
    				url : rootPath+"basedata/menuinfo!GetList.action",
    				timeout : 60000
    			}),
    			reader : new Ext.data.JsonReader({
    				totalProperty : 'count',
    		...
    		var tempArray = [
               ...
               { header: "菜单名称", sortable: true, dataIndex: 'C_NAME',width:30},
    		  ...
    		  if (btn) {
    			...
    			if(record&&control=='delete'){
    				if(window.confirm('是否确认删除？')){
						...
						loadMask.show();
						Ext.Ajax.request({
				            url : rootPath+"basedata/menuinfo!delete.action",
				            timeout : 120000,
