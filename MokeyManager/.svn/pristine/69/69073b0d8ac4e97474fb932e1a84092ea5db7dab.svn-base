 ...
 * 游戏帮：游戏推荐
 */
public class GameRecommendDaoImpl implements GameRecommendDao {

	private static final Logger LOGGER = Logger
			.getLogger(GameRecommendDaoImpl.class);
	...
	public Map<String, Object> getAllGame(int page, String name, String gameCate) {
		...
		try {
			StringBuffer sql = new StringBuffer("SELECT COUNT(1)" +
					...
					" WHERE T.C_ID = R.C_GID AND R.C_TYPE = 1)");
			...
			LOGGER.error("GameRecommendDaoImpl.getAllGame failed, e : "
					+ e);
		}
		return ret;
	}
	...
	public Map<String, Object> recommendGameList(int page, String name) {
		...
		try {
			StringBuffer sql = new StringBuffer("SELECT COUNT(1)" +
					" FROM T_GAME_APP_INFO T, T_GAME_APP_RECOMMEND R WHERE" +
			...
			LOGGER.error("GameRecommendDaoImpl.recommendList failed, e : " + e);
		}
		return ret;
	}
	...
	public int isExist(String gid) {
		try {
			String sql = "SELECT COUNT(1) FROM T_GAME_APP_RECOMMEND WHERE C_GID = ? AND C_TYPE = 1";
			return jdbcTemplate.queryForObject(sql, new Object[]{gid}, Integer.class);
		} catch (Exception e) {
			LOGGER.error("GameRecommendDaoImpl.isExist failed, e : " + e);
		}
		return 0;
	}
	...
	public void add(String gid, String order) {
		try {
			String sql = "INSERT INTO T_GAME_APP_RECOMMEND" +
					...
					" (SEQ_GAME_APP_RECOMMEND.NEXTVAL,?,1,?)";
			jdbcTemplate.update(sql, new Object[]{gid,order});
		} catch (Exception e) {
			LOGGER.error("GameRecommendDaoImpl.add failed, e : " + e);
		}
	}
	...
	public void update(String gid, String order) {
		try {
			...
			LOGGER.error("GameRecommendDaoImpl.update failed, e : " + e);
		}
	}
	...
	public void remove(String gid) {
		try {
			String sql = "DELETE FROM T_GAME_APP_RECOMMEND WHERE C_GID = ? AND C_TYPE = 1";
			jdbcTemplate.update(sql, new Object[]{gid});
		} catch (Exception e) {
			LOGGER.error("GameRecommendDaoImpl.remove failed, e : " + e);
		}
	}
