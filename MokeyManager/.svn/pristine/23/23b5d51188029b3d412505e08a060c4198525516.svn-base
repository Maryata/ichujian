...
<form id="form1" method="post" action="${pageContext.request.contextPath}/basedata/h5GameCategoryAction!toCateAdd.action">
...
$(function(){
	// 查询应用总数
	$.post("${pageContext.request.contextPath}/basedata/h5GameCategoryAction!gameCategoryList.action",
		function(data){
			...
			if(count != 0){
				...
				// 分页显示H5游戏分类
				getCatePage("${pageContext.request.contextPath}/basedata/h5GameCategoryAction!gameCategoryList.action");
			}
	});
	// 查询分类(第一页)
	$.post("${pageContext.request.contextPath}/basedata/h5GameCategoryAction!gameCategoryList.action",
...
function deleteCate(cid){
    ...
    if(flag){
		$.post("${pageContext.request.contextPath}/basedata/h5GameCategoryAction!delGameCate.action",
...
function modifyCate(cid,cname,order,logo){
	$("#form2").attr("action","${pageContext.request.contextPath}/basedata/h5GameCategoryAction!toUpdate.action"); 
	$("#editId").val(cid);
	$("#editName").val(cname);
	$("#editOrder").val(order);
	$("#editLogo").val(logo);
...
function handleCate(cid,cname){
	$("#form2").attr("action","${pageContext.request.contextPath}/basedata/h5GameCategoryAction!toHandle.action"); 
	$("#editId").val(cid);
	$("#editName").val(cname);
	$("#form2").submit();
}
