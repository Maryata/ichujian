...
public class ActivityContentTypeDaoImpl implements
ActivityContentTypeDao {
	...
	public Map<String, Object> GetList(String name, int start,
		...
		StringBuffer sql=new StringBuffer("select count(*) from T_ACTIVITY_CONTENT_TYPE t where t.c_islive=1 ");
	...
	public Map<String, Object> findName() {
		...
		String sqlString="select c_id,c_name from T_ACTIVITY_CONTENT_TYPE where c_islive=1 order by c_order";
	...
	public Map CheckName(String name, String id) {
		...
		if(StrUtils.isNotEmpty(name)){
			if(StrUtils.isNotEmpty(id)){
				sql="select count(*) from T_ACTIVITY_CONTENT_TYPE where c_name=? and c_id<>?";
				argsList.add(name);
				argsList.add(id);
			}else{
				sql="select count(*) from T_ACTIVITY_CONTENT_TYPE where c_name=?";
				argsList.add(name);
			}
	...
	public void Delete(String id) {
		// TODO Auto-generated method stub
		String sql="update T_ACTIVITY_CONTENT_TYPE set c_islive=0 where c_id=?";
	...
	public void Save(Map map) {
		...
		String sql="select * from T_ACTIVITY_CONTENT_TYPE t where t.c_id="+id_old;
		List cn  =jdbcTemplate.queryForList(sql);
		map.remove("c_id_old");
		if(cn.size()>0){
			Map wMap=new HashMap();
			wMap.put("c_id", id_old);
			JdbcTemplateUtils.updateDataByMap(jdbcTemplate, map, wMap, "T_ACTIVITY_CONTENT_TYPE");
		}else {
			String sqlid=JdbcTemplateUtils.getSeqId(jdbcTemplate, "SEQ_T_ACTIVITY_CONTENT_TYPE");
			map.put("c_id", sqlid);
			JdbcTemplateUtils.saveDataByMap(jdbcTemplate, map, "T_ACTIVITY_CONTENT_TYPE");
		}
	}
