 ...
 * 游戏帮：游戏礼包分类
 */
public class GameGiftCateDaoImpl implements GameGiftCateDao {
	
	private static final Logger LOGGER = Logger.getLogger(GameGiftCateDaoImpl.class);
	...
	public Map<String, Object> gameGiftCateList(int page) {
		...
		try {
			StringBuffer sql = new StringBuffer("SELECT COUNT(1)" +
					" FROM T_GAME_CATEGORY WHERE C_ISLIVE = 1 AND C_TYPE = '03'"); 
			...
			LOGGER.error("GameGiftCateDaoImpl.gameGiftCateList failed, e : " + e);
		}
		return ret;
	}
	...
	public void addGameGiftCate(String id, String name, String logo,
		...
		try {
			...
			LOGGER.error("GameGiftCateDaoImpl.addGameGiftCate failed, e : " + e);
		}
	}
	...
	public void delGameGiftCate(String id) {
		try {
			...
			LOGGER.error("GameGiftCateDaoImpl.delGameGiftCate failed, e : " + e);
		}
	}
	...
	public Map<String, Object> getAllGameGift(int page, String cid, String name, String giftCate) {
		...
		try {
			StringBuffer sql = new StringBuffer("SELECT COUNT(1)" +
					" FROM T_GAME_GIFTS_INFO T WHERE T.C_ISLIVE = 1" +
					" AND SYSDATE < T.C_EDATE AND NOT EXISTS" +
					" (SELECT C.C_GID FROM T_GAME_GIFTS_CATEGORY C WHERE" +
			...
			LOGGER.error("GameGiftCateDaoImpl.getAllGameGift failed, e : " + e);
		}
		return ret;
	}
	...
	public Map<String, Object> getCurrCateGift(int page, String cid, String name) {
		...
		try {
			StringBuffer sql = new StringBuffer("SELECT COUNT(1)" +
					" FROM T_GAME_GIFTS_CATEGORY T,T_GAME_GIFTS_INFO G WHERE " +
			...
			LOGGER.error("GameGiftCateDaoImpl.getCurrCateGift failed, e : " + e);
		}
		return ret;
	}
	...
	public List<Map<String,Object>> getGiftCateList() {
		try {
			...
			LOGGER.error("GameGiftCateDaoImpl.getGiftCateList failed, e : " + e);
		}
		return null;
	}
	...
	public int isExist(String cid, String gid) {
		try {
			String sql = "SELECT COUNT(1) FROM T_GAME_GIFTS_CATEGORY " +
					" WHERE C_CID = ? AND C_GID = ?";
			return jdbcTemplate.queryForObject(sql, new Object[]{cid, gid}, Integer.class);
		} catch (Exception e) {
			LOGGER.error("GameGiftCateDaoImpl.isExist failed, e : " + e);
		}
		return 0;
	}
	...
	public void add(String cid, String gid, String order) {
		try {
			String sql = "INSERT INTO T_GAME_GIFTS_CATEGORY" +
					" (C_ID, C_GID, C_CID, C_ORDER) VALUES" +
					" (SEQ_GAME_GIFTS_CATEGORY.NEXTVAL,?,?,?)";
			jdbcTemplate.update(sql, new Object[]{gid,cid,order});
		} catch (Exception e) {
			LOGGER.error("GameGiftCateDaoImpl.add failed, e : " + e);
		}
	}
	...
	public void update(String cid, String gid, String order) {
		try {
			String sql = "UPDATE T_GAME_GIFTS_CATEGORY SET" +
					" C_ORDER = ? WHERE C_CID = ? AND C_GID = ?";
			jdbcTemplate.update(sql, new Object[]{order,cid,gid});
		} catch (Exception e) {
			LOGGER.error("GameGiftCateDaoImpl.update failed, e : " + e);
		}
	}
	...
	public void remove(String cid, String gid) {
		try {
			String sql = "DELETE FROM T_GAME_GIFTS_CATEGORY" +
					" WHERE C_CID = ? AND C_GID = ?";
			jdbcTemplate.update(sql, new Object[]{cid,gid});
		} catch (Exception e) {
			LOGGER.error("GameGiftCateDaoImpl.remove failed, e : " + e);
		}
	}
