...
<table width="98%" height="35"  border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td width="62.7%" class="tz_crumbs"><img src="${pageContext.request.contextPath}/images/crumbs.jpg" width="9" height="9" /> 系统信息 -&gt; 3号键管理 -&gt; 礼包分类维护 </td>
  </tr>
</table>

<form id="form1" method="post" action="${pageContext.request.contextPath}/basedata/ekey/eKGameGiftCateAction!toCateAdd.action">
...
$(function(){
	// 查询应用总数
	$.post("${pageContext.request.contextPath}/basedata/ekey/eKGameGiftCateAction!gameGiftCateList.action",
		function(data){
			...
			if(count != 0){
				...
				// 分页显示礼包分类
				getCatePage("${pageContext.request.contextPath}/basedata/ekey/eKGameGiftCateAction!gameGiftCateList.action");
			}
	});
	// 查询分类(第一页)
	$.post("${pageContext.request.contextPath}/basedata/ekey/eKGameGiftCateAction!gameGiftCateList.action",
...
function deleteCate(cid){
    ...
    if(flag){
		$.post("${pageContext.request.contextPath}/basedata/ekey/eKGameGiftCateAction!delGameGiftCate.action",
...
function modifyCate(cid,cname,order,logo){
	$("#form2").attr("action","${pageContext.request.contextPath}/basedata/ekey/eKGameGiftCateAction!toUpdate.action"); 
	$("#editId").val(cid);
	$("#editName").val(cname);
	$("#editOrder").val(order);
	$("#editLogo").val(logo);
...
function handleCate(cid,cname){
	$("#form2").attr("action","${pageContext.request.contextPath}/basedata/ekey/eKGameGiftCateAction!toHandle.action"); 
	$("#editId").val(cid);
	$("#editName").val(cname);
	$("#form2").submit();
}
