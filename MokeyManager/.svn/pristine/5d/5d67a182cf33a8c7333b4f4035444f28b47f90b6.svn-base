...
<form id="form1" method="post" action="${pageContext.request.contextPath}/basedata/gameCategoryAction!toCateAdd.action">
...
$(function(){
	// 查询应用总数
	$.post("${pageContext.request.contextPath}/basedata/gameCategoryAction!gameCategoryList.action",
		function(data){
			...
			if(count != 0){
				...
				// 分页显示游戏分类
				getCatePage("${pageContext.request.contextPath}/basedata/gameCategoryAction!gameCategoryList.action");
			}
	});
	// 查询分类(第一页)
	$.post("${pageContext.request.contextPath}/basedata/gameCategoryAction!gameCategoryList.action",
...
function deleteCate(cid){
    ...
    if(flag){
		$.post("${pageContext.request.contextPath}/basedata/gameCategoryAction!delGameCate.action",
...
function modifyCate(cid,cname,order,logo){
	$("#form2").attr("action","${pageContext.request.contextPath}/basedata/gameCategoryAction!toUpdate.action"); 
	$("#editId").val(cid);
	$("#editName").val(cname);
	$("#editOrder").val(order);
	$("#editLogo").val(logo);
...
function handleCate(cid,cname){
	$("#form2").attr("action","${pageContext.request.contextPath}/basedata/gameCategoryAction!toHandle.action"); 
	$("#editId").val(cid);
	$("#editName").val(cname);
	$("#form2").submit();
}
