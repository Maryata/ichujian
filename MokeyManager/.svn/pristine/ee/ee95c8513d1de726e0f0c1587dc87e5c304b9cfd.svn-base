 ...
 * 游戏帮：游戏攻略
 */
public class GameStrategyDaoImpl implements GameStrategyDao {

private static final Logger LOGGER = Logger.getLogger(GameStrategyDaoImpl.class);
		...
		try {
			if(StrUtils.isNotEmpty(page)){
				StringBuffer sb = new StringBuffer("SELECT T.C_ID ID,G.C_NAME GNAME," +
						"T.C_NAME CNAME FROM T_GAME_STRATEGY_INFO T, T_GAME_APP_INFO G" +
			...
			LOGGER.error("gameStrategyDaoImpl.gameStrategyList failed, e : " + e);
		}
		return null;
	}
	...
	public Integer getTotal(String gid, String name, String gName) {
		try {
			StringBuffer sb = new StringBuffer("SELECT COUNT(*) FROM " +
					"T_GAME_STRATEGY_INFO T, T_GAME_APP_INFO G WHERE T.C_ISLIVE = 1 " +
			...
			LOGGER.error("gameStrategyDaoImpl.getTotalCol failed, e : " + e);
		}
		return null;
	}
	...
	public void deleteGameStrategy(String id) {
		try {
			String sql = "DELETE FROM T_GAME_STRATEGY_INFO WHERE C_ID = ?";
	...
	public List<Map<String, Object>> queryStrategyById(String editId) {
		try {
			String sql = "SELECT * FROM T_GAME_STRATEGY_INFO T WHERE T.C_ISLIVE = 1 AND T.C_ID = ?";
	...
	public void updateGameStrategy(List<Object> args) {
		try {
			String sql = "UPDATE T_GAME_STRATEGY_INFO T SET T.C_GID = ?, T.C_NAME = ?, " +
					"T.C_DEPICT = ?, T.C_DATE = ?, T.C_ISLIVE = ?, T.C_ORDER = ?, C_SHAREURL = ?, C_LOGOURL = ? WHERE T.C_ID = ?";
			this.jdbcTemplate.update(sql,args.toArray());
		} catch (Exception e) {
			LOGGER.error("gameStrategyDaoImpl.updateGameStrategy failed, e : " + e);
		}
	}
	...
	public Integer scanedNum(String newId, String gid) {
		try {
			...
			LOGGER.error("gameStrategyDaoImpl.scanedNum failed, e : " + e);
		}
		return null;
	}
	...
	public boolean existId(String id) {
		try {
			String sql = "SELECT COUNT(*) FROM T_GAME_STRATEGY_INFO T WHERE T.C_ID = ? ";
	...
	public List<Map<String, Object>> selectDepict() {
		try {
			StringBuffer sql = new StringBuffer("SELECT T.C_ID ID,T.C_DEPICT DECIPT" +
					" FROM T_EK_GAME_STRATEGY_INFO T WHERE T.C_ISLIVE = 1");
			//DaoUtil.addfy_oracle(sql, 0, 10);
			return this.jdbcTemplate.queryForList(sql.toString());
		} catch (Exception e) {
			LOGGER.error("gameStrategyDaoImpl.selectDepict failed, e : " + e);
		}
		return null;
	}
	...
	public int setImgSize(String id, String depict, String flag) {
		try {
			String sql = "UPDATE T_GAME_INFORMATION_INFO T SET T.C_DEPICT = ?, T.C_CHANGE = ? WHERE T.C_ID = ?";
