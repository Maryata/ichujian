 ...
 * 游戏帮：游戏活动
 */
@Controller("eKGameActAction")
public class EKGameActAction extends AbstractAction {

    // 游戏活动
    @Autowired
    private EKGameActService eKGameActService;
    ...
    public String toActList() {
        log.info("into EKGameActAction.toActList");
        try {
            // 查询所有游戏
            ...
            log.error("EKGameActAction.toActList failed, e : " + e);
        }
        return "gameActList";
    }
    ...
    public String GameActList() {
        ...
        log.info("into EKGameActAction.GameActList");
        ...
        try {
            ...
            List<Map<String, Object>> list = eKGameActService.gameActList(gid, name, page, gName);
            super.writeJSONToResponse(list);
        } catch (Exception e) {
            log.error("EKGameActAction.GameActList failed, e : " + e);
        }
        return NONE;
    }
    ...
    public String getTotal() {
        ...
        String gName = getParameter("gName");// 游戏名
        String name = getParameter("name");// 活动名
        log.info("into EKGameActAction.getTotalCol");
        log.info("gid = " + gid + ", name = " + name);
        try {
            // 总条数
            Integer total = eKGameActService.getTotal(gid, name, gName);
            // 计算总页数
            ...
            log.error("EKGameActAction.getTotal failed,", e);
        }
        return NONE;
    }
    ...
    public String deleteGameAct() {
        ...
        log.info("into EKGameActAction.GameActAction");
        log.info("id = " + id);
        try {
            // 删除
            eKGameActService.deleteGameAct(id);
        } catch (Exception e) {
    ...
    public String toGameActAdd() {
        log.info("into EKGameActAction.toGameActAdd");
        try {
            // 查询所有游戏
            @SuppressWarnings("unchecked")
            List<Map<String, Object>> games = appInfoService.getGameAppInfoList();
            ...
            log.error("EKGameActAction.toGameActAdd failed, e : " + e);
        }
        return "toGameActAdd";
    }
    ...
    public String toGameActEdit() {
        ...
        log.info("into EKGameActAction.toGameActEdit");
        ...
        try {
            ...
            getRequest().setAttribute("games", games);
            // 根据id查询活动
            List<Map<String, Object>> list = eKGameActService.queryActById(editId);
            String editGid = "";
            String editOrder = "";
            String editName = "";
            String editDepict = "";
            ...
            log.error("EKGameActAction.toGameActEdit failed, e : " + e);
        }
        return "toGameActAdd";
    }
    ...
    public String saveGameAct() {
        ...
        log.info("into EKGameActAction.saveGameAct");
        log.info("id = " + id + ", gid = " + gid + ", order = " + order + ", name = "
                + name + ", depict = " + depict + ", logourl = " + logourl +
                ", sDate = " + sDate + ", eDate = " + eDate + ", reward = " + reward);
        try {
            eKGameActService.saveGameAct(id, gid, order, name, depict, logourl, sDate, eDate, reward);
        } catch (Exception e) {
            log.error("EKGameActAction.saveGameAct failed, e : " + e);
        }
        return "saveGameAct";
    }
    ...
    public String ajaxList() {
        ...
        try {
            retMap = eKGameActService.listUser(start, limit, aid, flag);
            retMap.put("status", "Y");
        } catch (Exception e) {
            retMap.put("status", "N");
    ...
    public String ajaxSave() {
        ...
        try {
            eKGameActService.updateUser(map);
            map.put("status", "Y");
