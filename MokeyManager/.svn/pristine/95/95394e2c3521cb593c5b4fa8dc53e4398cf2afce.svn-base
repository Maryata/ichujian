...
function gameList(){
	...
	// 查询非当前分类的其他所有游戏
	$.post("${pageContext.request.contextPath}/basedata/ekey/eKGameCategoryAction!getAllGame.action",{cid:cid,name:name,gameCate:gameCate},
		function(data){
			var json = eval("("+data+")");
			...
			if(json != 0){
				...
				// 所有游戏
				getGamePage("${pageContext.request.contextPath}/basedata/ekey/eKGameCategoryAction!getAllGame.action",cid,name,gameCate);
			}
	});
	$.post("${pageContext.request.contextPath}/basedata/ekey/eKGameCategoryAction!getAllGame.action",{cid:cid,name:name,gameCate:gameCate},
		function(data){
...
function cateGameList(){
	...
	// 查询当前分类的游戏总数
	$.post("${pageContext.request.contextPath}/basedata/ekey/eKGameCategoryAction!getCurrCateGame.action",{cid:cid,name:name},
		function(data){
			var json = eval("("+data+")");
			var count = json[0].count;// 数据总数
			$("#page22").empty();
			if(json != 0){
				...
				// 当前分类中的游戏
				currCateGamePage("${pageContext.request.contextPath}/basedata/ekey/eKGameCategoryAction!getCurrCateGame.action",cid,name);
			}
	});
	$.post("${pageContext.request.contextPath}/basedata/ekey/eKGameCategoryAction!getCurrCateGame.action",{cid:cid,name:name},
		function(data){
...
function submitt(){
	...
	$.post("${pageContext.request.contextPath}/basedata/ekey/eKGameCategoryAction!handleCate.action",{cid:cid,gid:gid,removeGid:removeGid,order:order},function(data){
