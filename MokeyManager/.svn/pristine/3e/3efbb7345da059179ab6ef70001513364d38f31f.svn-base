...
<table width="98%" height="35"  border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td width="62.7%" class="tz_crumbs"><img src="${pageContext.request.contextPath}/images/crumbs.jpg" width="9" height="9" /> 系统信息 -&gt; e键-游戏 -&gt; 游戏攻略分类维护 </td>
  </tr>
</table>

<form id="form1" method="post" action="${pageContext.request.contextPath}/basedata/ekey/eKGameStrategyCateAction!toStrategyCateAdd.action">
...
$(function(){
	// 查询应用总数
	$.post("${pageContext.request.contextPath}/basedata/ekey/eKGameStrategyCateAction!gameStrategyCateList.action",
		function(data){
			...
			if(count != 0){
				...
				// 分页显示游戏攻略分类分类
				getCatePage("${pageContext.request.contextPath}/basedata/ekey/eKGameStrategyCateAction!gameStrategyCateList.action");
			}
	});
	// 查询分类(第一页)
	$.post("${pageContext.request.contextPath}/basedata/ekey/eKGameStrategyCateAction!gameStrategyCateList.action",
...
function deleteCate(cid){
    ...
    if(flag){
		$.post("${pageContext.request.contextPath}/basedata/ekey/eKGameStrategyCateAction!delGameStrategyCate.action",
...
function modifyCate(cid,cname,order,logo){
	$("#form2").attr("action","${pageContext.request.contextPath}/basedata/ekey/eKGameStrategyCateAction!toUpdate.action");
	$("#editId").val(cid);
	$("#editName").val(cname);
	$("#editOrder").val(order);
	$("#editLogo").val(logo);
...
function handleCate(cid,cname){
	$("#form2").attr("action","${pageContext.request.contextPath}/basedata/ekey/eKGameStrategyCateAction!toHandle.action");
	$("#editId").val(cid);
	$("#editName").val(cname);
	$("#form2").submit();
}
