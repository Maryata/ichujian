...
public class McrAppImport {
	...
	public static void impPhoneErea() throws IOException, InvalidFormatException {
		for (int j = 1; j < 7; j++){
			Workbook wb = WorkbookFactory.create(loader.getResourceAsStream(j + ".xlsx"));

			final String sql = "INSERT INTO T_BASE_PHONE_EREA (C_ID, C_PREFIX, C_PHONE, C_PROVINCE, C_CITY, C_ISP, C_CODE , C_ZIP, C_TYPE)" +
					" VALUES (SEQ_BASE_PHONE_AREA.NEXTVAL,?,?,?,?,?,?,?,?)";
			...
			for ( int i = 0; i < numberOfSheets; ++i ) {
				...
				for ( int rowNum = rowStart; rowNum < rowEnd; ++rowNum ) {
					...
					if(lastColumn != 8) {
						Comment cellComment = r.getCell(1).getCellComment();
						System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" + cellComment);
						continue;
					}

					Object[] objArray = new Object[lastColumn];
					...
					if(null != objArray[0]) {
						batchArgs.add( objArray );
					}

				}

				jdbcTemplate.batchUpdate( sql, batchArgs );
				System.out.println("app count -->" + batchArgs.size());
			}
		}
