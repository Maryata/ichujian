 ...
 */
@Repository
public class EKGameCollectionDaoImpl implements EKGameCollectionDao {

    private static final Logger LOGGER = Logger.getLogger(EKGameCollectionDaoImpl.class);
    ...
    public List<Map<String, Object>> gameCol(String cid) {
        try {
            ...
            String sql = "SELECT * FROM T_EK_GAME_COLLECTION_BASE T WHERE 1 = 1";
            sb.append(sql);
            ...
            LOGGER.error("EKGameCollectionDaoImpl.gameCol failed, e : " + e);
        }
        return null;
    }
    ...
    public void addCol(String name, String type) {
        try {
            if (StrUtils.isNotEmpty(name) && StrUtils.isNotEmpty(type)) {
                ...
                String sql = "INSERT INTO T_EK_GAME_COLLECTION_BASE " +
                        "(C_ID,C_NAME,C_TNAME,C_TYPE,C_ISLIVE) " +
                        "VALUES(SEQ_EK_GAME_COLLECTION_BASE.NEXTVAL,?,?,?,1)";
                Object[] obj = {name, tbName, type};
                this.jdbcTemplate.update(sql, obj);
            }
        } catch (DataAccessException e) {
            LOGGER.error("EKGameCollectionDaoImpl.addCol failed, e : " + e);
        }
    }
    ...
    public void deleteColGame(String cid) {
        try {
            String sql = "DELETE FROM T_EK_GAME_COLLECTION_DETAIL T WHERE T.C_CID = ?";
            Object[] obj = {cid};
            this.jdbcTemplate.update(sql, obj);
        } catch (Exception e) {
            LOGGER.error("EKGameCollectionDaoImpl.deleteColGame failed, e : " + e);
        }
    }
    ...
    public void deleteCol(String cid) {
        try {
            if (StrUtils.isNotEmpty(cid)) {
                String sql = "DELETE FROM T_EK_GAME_COLLECTION_BASE T WHERE T.C_ID = ?";
                Object[] obj = {cid};
                this.jdbcTemplate.update(sql, obj);
            }
        } catch (DataAccessException e) {
            LOGGER.error("EKGameCollectionDaoImpl.deleteCol failed, e : " + e);
        }
    }
    ...
    public void modifyCol(String cid, String cname, String islive) {
        try {
            if (StrUtils.isNotEmpty(cid)) {
                String sql = "UPDATE T_EK_GAME_COLLECTION_BASE T SET T.C_NAME = ?, T.C_ISLIVE = ? WHERE T.C_ID = ?";
                Object[] obj = {cname, islive, cid};
                this.jdbcTemplate.update(sql, obj);
            }
        } catch (DataAccessException e) {
            LOGGER.error("EKGameCollectionDaoImpl.modifyCol failed, e : " + e);
        }
    }
    ...
    public void isvalid(String cid, String islive) {
        try {
            if (StrUtils.isNotEmpty(cid) && StrUtils.isNotEmpty(islive)) {
                String sql = "UPDATE T_EK_GAME_COLLECTION_BASE T SET T.C_ISLIVE = ? WHERE T.C_ID = ?";
                Object[] obj = {islive, cid};
                this.jdbcTemplate.update(sql, obj);
            }
        } catch (DataAccessException e) {
            LOGGER.error("EKGameCollectionDaoImpl.isvalid failed, e : " + e);
        }
    }
    ...
    public List<Map<String, Object>> getGameList(int page, String cid, String type) {
        try {
            if (StrUtils.isNotEmpty(page)) {
                ...
                sql.append("AND NOT EXISTS ( SELECT 1 FROM T_EK_GAME_COLLECTION_DETAIL" +
...
//						" FROM T_EK_GAME_APP_INFO T WHERE T.C_ISLIVE = 1  AND NOT EXISTS ( " +
//						" SELECT 1 FROM T_EK_GAME_COLLECTION_DETAIL C WHERE C.C_ISLIVE = 1 AND" +
//						" T.C_ID = C.C_GID");
...
//				}
            ...
            LOGGER.error("EKGameCollectionDaoImpl.getGameList failed, e : " + e);
        }
        return null;
    }
    ...
    public Integer getTotal(String cid, String type) {
        try {
            ...
            StringBuffer sql = new StringBuffer("SELECT COUNT(C_ID) FROM (SELECT T.C_ID FROM " + tbName +
                    ...
                    " T_EK_GAME_COLLECTION_DETAIL C WHERE T.C_ID = C.C_GID ");
            if ("1".equals(type)) {
                sql.append(" AND T.C_TYPE <> 3 ");
            }
            if ("2".equals(type)) {
...
//			StringBuffer sql = new StringBuffer("SELECT COUNT(C_ID) FROM (SELECT T.C_ID FROM T_EK_GAME_APP_INFO" +
//					" T WHERE T.C_ISLIVE = 1 AND NOT EXISTS (SELECT C.C_GID FROM" +
//					" T_EK_GAME_COLLECTION_DETAIL C WHERE T.C_ID = C.C_GID");
            // 如果是合集1或5，则查询不属于当前合集的所有游戏
//			if("1".equals(cid) || "5".equals(cid)){
...
//			}
            ...
            LOGGER.error("EKGameCollectionDaoImpl.getTotal failed, e : " + e);
        }
        return null;
    }
    ...
    public void handleCol(String cid, String gid, String order) {
        try {
            if (StrUtils.isNotEmpty(cid) && StrUtils.isNotEmpty(gid)) {
                String sql = "INSERT INTO T_EK_GAME_COLLECTION_DETAIL" +
                        " T(T.C_ID,T.C_GID,T.C_CID,T.C_ISLIVE,T.C_ORDER) " +
                        "VALUES (SEQ_EK_GAME_COLLECTION_DETAIL.NEXTVAL,?,?,1,?)";
                Object[] obj = {gid, cid, order};
                this.jdbcTemplate.update(sql, obj);
            }
        } catch (DataAccessException e) {
            LOGGER.error("EKGameCollectionDaoImpl.handleCol failed, e : " + e);
        }
    }
    ...
    public Integer isExist(String cid, String gid) {
        try {
            String sql = "SELECT COUNT(*) FROM T_EK_GAME_COLLECTION_DETAIL T WHERE T.C_ISLIVE = 1 AND T.C_CID = ? AND T.C_GID = ?";
            Object[] obj = {cid, gid};
            return this.jdbcTemplate.queryForObject(sql, obj, Integer.class);
        } catch (DataAccessException e) {
            LOGGER.error("EKGameCollectionDaoImpl.isExist failed, e : " + e);
        }
        return null;
    }
...
//				list.add(maxSize);
//			}
            sql.append(" AND NOT EXISTS (SELECT 1 FROM T_EK_GAME_COLLECTION_DETAIL C" +
...
//			}
            ...
            LOGGER.error("EKGameCollectionDaoImpl.queryGameCondition failed, e : " + e);
        }
        return null;
    }
...
//				list.add(maxSize);
//			}
            sql.append(" AND NOT EXISTS (SELECT C.C_GID FROM T_EK_GAME_COLLECTION_DETAIL " +
...
//			}
            ...
            LOGGER.error("EKGameCollectionDaoImpl.getTotalCondition failed, e : " + e);
        }
        return null;
    }
    ...
    public List<Map<String, Object>> getGamePageByColId(String cid, int page, String type) {
        try {
            ...
            StringBuffer sb = new StringBuffer("SELECT A.C_ID C_ID," +
                    " A.C_NAME C_NAME, C.C_ORDER C_ORDER FROM T_EK_GAME_COLLECTION_DETAIL" +
            ...
            LOGGER.error("EKGameCollectionDaoImpl.getGamePageByColId failed, e : " + e);
        }
        return null;
    }
        ...
        try {
            if (StrUtils.isNotEmpty(cid)) {
                ...
                StringBuffer sql = new StringBuffer("SELECT COUNT(*) FROM " + tbName + " T," +
                        " T_EK_GAME_COLLECTION_DETAIL C WHERE T.C_ISLIVE = 1 AND" +
            ...
            LOGGER.error("EKGameCollectionDaoImpl.getTotalCol failed, e : " + e);
        }
        return null;
    }
    ...
    public void removeGame(String cid, String gid) {
        try {
            String sql = "DELETE FROM T_EK_GAME_COLLECTION_DETAIL T WHERE T.C_CID = ? AND T.C_GID = ? AND T.C_ISLIVE = 1";
            Object[] obj = {cid, gid};
            this.jdbcTemplate.update(sql, obj);
        } catch (Exception e) {
            LOGGER.error("EKGameCollectionDaoImpl.removeGame failed, e : " + e);
        }
    }
    ...
    public void updateOrder(String cid, String gid, String order) {
        try {
            String sql = "UPDATE T_EK_GAME_COLLECTION_DETAIL T SET T.C_ORDER = ? " +
                    "WHERE T.C_GID = ? AND T.C_CID = ? AND T.C_ISLIVE = 1";
            Object[] obj = {order, gid, cid};
            this.jdbcTemplate.update(sql, obj);
        } catch (Exception e) {
            LOGGER.error("EKGameCollectionDaoImpl.updateOrder failed, e : " + e);
        }
    }
    ...
    public Integer getColTotalCondition(String cid, String name, String type) {
        try {
            ...
            StringBuffer sql = new StringBuffer("SELECT COUNT(1) FROM" +
                    " T_EK_GAME_COLLECTION_DETAIL C, " + tbName + " T WHERE C.C_ISLIVE = 1" +
            ...
            LOGGER.error("EKGameCollectionDaoImpl.getColTotalCondition failed, e : " + e);
        }
        return null;
    }
    ...
    public List<Map<String, Object>> queryColGameCondition(String cid,
        ...
        try {
            ...
            StringBuffer sql = new StringBuffer("SELECT C_ID, C_NAME " +
                    ...
                    " C.C_ORDER FROM " + tbName + " T, T_EK_GAME_COLLECTION_DETAIL C  WHERE T.C_ISLIVE = 1 " +
            ...
            LOGGER.error("EKGameCollectionDaoImpl.queryColGameCondition failed, e : " + e);
        }
        return null;
    }
}
