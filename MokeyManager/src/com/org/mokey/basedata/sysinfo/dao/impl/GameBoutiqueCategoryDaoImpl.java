 ...
 * 游戏帮：游戏精品分类
 */
public class GameBoutiqueCategoryDaoImpl implements GameBoutiqueCateDao {

	private static final Logger LOGGER = Logger
			.getLogger(GameBoutiqueCategoryDaoImpl.class);
	...
	@Override
	// 获取游戏分类（下拉菜单）
	public List<Map<String, Object>> getGameBoutiqueCateList() {
		try {
			String sql = "SELECT C_ID, C_NAME FROM T_GAME_CATEGORY" +
					" WHERE C_ISLIVE = 1 AND C_TYPE = '01' ORDER BY C_ORDER";
			return jdbcTemplate.queryForList(sql);
		} catch (DataAccessException e) {
			LOGGER.error("GameBoutiqueCategoryDaoImpl.getGameBoutiqueCategoryList failed, e : " + e);
		}
		return null;
	}
	...
	@Override
	// 分页显示游戏分类
	public Map<String, Object> gameBoutiqueCateList(int page) {
		Map<String, Object> ret = new HashMap<String, Object>();
		try {
			StringBuffer sql = new StringBuffer("SELECT COUNT(1)" +
					" FROM T_GAME_CATEGORY WHERE C_ISLIVE = 1 AND C_TYPE = '02'"); //  02 精品分类 
			...
			LOGGER.error("GameBoutiqueCategoryDaoImpl.gameBoutiqueCategoryList failed, e : " + e);
		}
		return ret;
	}
	...
	public void addGameBoutiqueCate(String id, String name, String logo,
		...
		try {
			...
			LOGGER.error("GameBoutiqueCategoryDaoImpl.addGameBoutiqueCategory failed, e : " + e);
		}
	}
	...
	public void delGameBoutiqueCate(String id) {
		try {
			...
			LOGGER.error("GameBoutiqueCategoryDaoImpl.delGameBoutiqueCategory failed, e : " + e);
		}
	}
	...
	public Map<String, Object> getAllGame(int page, String cid, String name,
		...
		try {
			StringBuffer sql = new StringBuffer("SELECT COUNT(1)" +
					" FROM T_GAME_APP_INFO T WHERE T.C_ISLIVE = 1" +
					" AND NOT EXISTS" +
					" (SELECT C.C_GID FROM t_game_boutique_category C WHERE" +
			...
			LOGGER.error("GameBoutiqueCategoryDaoImpl.getAllGame failed, e : " + e);
		}
		return ret;
	}
	...
	public Map<String, Object> getCurrCateGame(int page, String cid, String name) {
		...
		try {
			StringBuffer sql = new StringBuffer("SELECT COUNT(1)" +
					" FROM t_game_boutique_category T,T_GAME_APP_INFO G WHERE " +
			...
			LOGGER.error("GameBoutiqueCategoryDaoImpl.getCurrCateGift failed, e : " + e);
		}
		return ret;
	}
	...
	public int isExist(String cid, String gid) {
		try {
			String sql = "SELECT COUNT(1) FROM t_game_boutique_category " +
					" WHERE C_CID = ? AND C_GID = ?";
			return jdbcTemplate.queryForObject(sql, new Object[]{cid, gid}, Integer.class);
		} catch (Exception e) {
			LOGGER.error("GameBoutiqueCategoryDaoImpl.isExist failed, e : " + e);
		}
		return 0;
	}
	...
	public void add(String cid, String gid, String order) {
		try {
			String sql = "INSERT INTO t_game_boutique_category" +
					" (C_ID, C_GID, C_CID, C_ORDER) VALUES" +
					" (seq_game_boutique_category.NEXTVAL,?,?,?)";
			jdbcTemplate.update(sql, new Object[]{gid,cid,order});
		} catch (Exception e) {
			LOGGER.error("GameBoutiqueCategoryDaoImpl.add failed, e : " + e);
		}
	}
	...
	public void update(String cid, String gid, String order) {
		try {
			String sql = "UPDATE t_game_boutique_category SET" +
					" C_ORDER = ? WHERE C_CID = ? AND C_GID = ?";
			jdbcTemplate.update(sql, new Object[]{order,cid,gid});
		} catch (Exception e) {
			LOGGER.error("GameBoutiqueCategoryDaoImpl.update failed, e : " + e);
		}
	}
	...
	public void remove(String cid, String gid) {
		try {
			String sql = "DELETE FROM t_game_boutique_category" +
					" WHERE C_CID = ? AND C_GID = ?";
			jdbcTemplate.update(sql, new Object[]{cid,gid});
		} catch (Exception e) {
			LOGGER.error("GameBoutiqueCategoryDaoImpl.remove failed, e : " + e);
		}
	}
}
