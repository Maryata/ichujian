...
private static final Logger LOGGER = Logger.getLogger(GameInfoDaoImpl.class);
		...
		try {
			if(StrUtils.isNotEmpty(page)){
				StringBuffer sb = new StringBuffer("SELECT T.C_ID ID,G.C_NAME GNAME," +
						"T.C_NAME CNAME FROM T_GAME_INFORMATION_INFO T, T_GAME_APP_INFO G" +
			...
			LOGGER.error("GameInfoDaoImpl.getGameList failed, e : " + e);
		}
		return null;
	}
	...
	public Integer getTotal(String gid, String name, String gName) {
		try {
			StringBuffer sb = new StringBuffer("SELECT COUNT(*) FROM " +
					"T_GAME_INFORMATION_INFO T, T_GAME_APP_INFO G WHERE T.C_ISLIVE = 1 " +
			...
			LOGGER.error("GameInfoDaoImpl.getTotalCol failed, e : " + e);
		}
		return null;
	}
	...
	public void deleteGameInfo(String id) {
		try {
			String sql = "DELETE FROM T_GAME_INFORMATION_INFO WHERE C_ID = ?";
	...
	public List<Map<String, Object>> queryInfoById(String editId) {
		try {
			String sql = "SELECT * FROM T_GAME_INFORMATION_INFO T WHERE T.C_ISLIVE = 1 AND T.C_ID = ?";
	...
	public void updateGameInfo(List<Object> args) {
		try {
			String sql = "UPDATE T_GAME_INFORMATION_INFO T SET T.C_GID = ?, T.C_NAME = ?, " +
					"T.C_DEPICT = ?, T.C_DATE = ?, T.C_ISLIVE = ?, T.C_ORDER = ?, C_SHAREURL = ? , C_LOGOURL = ? WHERE T.C_ID = ?";
			this.jdbcTemplate.update(sql,args.toArray());
		} catch (Exception e) {
			LOGGER.error("GameInfoDaoImpl.updateGameInfo failed, e : " + e);
		}
	}
	...
	public Integer scanedNum(String newId, String gid) {
		try {
			...
			LOGGER.error("GameInfoDaoImpl.scanedNum failed, e : " + e);
		}
		return null;
	}
	...
	public boolean existId(String id) {
		try {
			String sql = "SELECT COUNT(*) FROM T_GAME_INFORMATION_INFO T WHERE T.C_ID = ? ";
