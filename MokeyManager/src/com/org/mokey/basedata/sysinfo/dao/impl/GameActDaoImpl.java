...
public class GameActDaoImpl implements GameActDao {

private static final Logger LOGGER = Logger.getLogger(GameActDaoImpl.class);
		...
		try {
			if(StrUtils.isNotEmpty(page)){
				StringBuffer sb = new StringBuffer("SELECT T.C_ID ID,G.C_NAME GNAME," +
						"T.C_NAME CNAME FROM T_GAME_ACTIVITY_INFO T, T_GAME_APP_INFO G" +
			...
			LOGGER.error("gameActDaoImpl.gameActList failed, e : " + e);
		}
		return null;
	}
	...
	public Integer getTotal(String gid, String name, String gName) {
		try {
			StringBuffer sb = new StringBuffer("SELECT COUNT(*) FROM " +
					"T_GAME_ACTIVITY_INFO T, T_GAME_APP_INFO G WHERE T.C_ISLIVE = 1 " +
			...
			LOGGER.error("gameActDaoImpl.getTotalCol failed, e : " + e);
		}
		return null;
	}
	...
	public void deleteGameAct(String id) {
		try {
			String sql = "DELETE FROM T_GAME_ACTIVITY_INFO WHERE C_ID = ?";
	...
	public List<Map<String, Object>> queryActById(String editId) {
		try {
			String sql = "SELECT * FROM T_GAME_ACTIVITY_INFO T WHERE T.C_ISLIVE = 1 AND T.C_ID = ?";
	...
	public void saveGameAct(List<Object> args) {
		try {
			String sql = "INSERT INTO T_GAME_ACTIVITY_INFO " +
			...
			LOGGER.error("gameActDaoImpl.saveGameAct failed, e : " + e);
		}
	}
	...
	public void updateGameAct(List<Object> args) {
		try {
			String sql = "UPDATE T_GAME_ACTIVITY_INFO T SET" +
			...
			LOGGER.error("gameActDaoImpl.updateGameAct failed, e : " + e);
		}
	}
	...
	public boolean existId(String id) {
		try {
			String sql = "SELECT COUNT(*) FROM T_GAME_ACTIVITY_INFO T WHERE T.C_ID = ? ";
	...
	public Map<String, Object> listUser(int start, int limit, int aid,int flag) {
		...
		StringBuilder sql = new StringBuilder("select count(1) from T_GAME_MEMBER_ACTIVITY where c_aid=?");
		StringBuilder sql0 = new StringBuilder("select t1.c_nickname,t2.c_name,t.c_comment,t.c_uid,t.c_id,t.c_img from T_GAME_MEMBER_ACTIVITY t left join t_game_member_info t1 on t.c_uid=t1.c_id left join t_game_activity_info t2 on t.c_aid=t2.c_id where t.c_aid=? and t1.c_state=1");
		if(0 == flag) {
	...
	public void updateUser(Map<String, Object> map) throws Exception {
		...
		String seqName = "seq_game_member_task";
        ...
        // 更新用户参与活动的表记录积分
        jdbcTemplate.update("update t_game_member_activity set c_score=? where c_id=?", Integer.parseInt(score),memberActivityId);
        // 插入用户获取任务积分
        jdbcTemplate.update("insert into t_game_member_task (c_id, c_uid, c_tid, c_count, c_date, c_strdate) values (?, ?, ?, ?,?,?)",newId,uid,tid,score, currentDate, String.valueOf(currentDate.getTime()));
        // 更新用户总积分
        jdbcTemplate.update("update t_game_member_score set c_score = c_score + ? where c_uid = ?",score,uid);
