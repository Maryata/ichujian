 ...
 * 游戏帮：h5游戏精品分类
 */
public class H5GameBoutiqueCategoryDaoImpl implements H5GameBoutiqueCategoryDao {

	private static final Logger LOGGER = Logger
			.getLogger(H5GameBoutiqueCategoryDaoImpl.class);
	...
	@Override
	// 获取游戏分类（下拉菜单）
	public List<Map<String, Object>> getBoutiqueGameCategoryList() {
		try {
			String sql = "SELECT C_ID, C_NAME FROM T_GAME_CATEGORY" +
					" WHERE C_ISLIVE = 1 AND C_TYPE = '01' ORDER BY C_ORDER";
			return jdbcTemplate.queryForList(sql);
		} catch (DataAccessException e) {
			LOGGER.error("H5GameBoutiqueCategoryDaoImpl.getGameBoutiqueCategoryList failed, e : " + e);
		}
		return null;
	}
	...
	public Map<String, Object> gameBoutiqueCategoryList(int page) {
		...
		try {
			StringBuffer sql = new StringBuffer("SELECT COUNT(1)" +
					" FROM T_GAME_CATEGORY WHERE C_ISLIVE = 1 AND C_TYPE = '05'"); // 05 H5精品分类
			...
			LOGGER.error("H5GameBoutiqueCategoryDaoImpl.gameBoutiqueCategoryList failed, e : " + e);
		}
		return ret;
	}
	...
	@Override
	// 新增游戏游戏分类
	public void addGameBoutiqueCategory(String id, String name, String logo,
		...
		try {
			...
			LOGGER.error("H5GameBoutiqueCategoryDaoImpl.addGameBoutiqueCategory failed, e : " + e);
		}
	}

	@Override
	// 删除分类
	public void delGameBoutiqueCategory(String id) {
		try {
			String sql = "DELETE FROM T_GAME_CATEGORY WHERE C_ID = ?";
			this.jdbcTemplate.update(sql,new Object[]{id});
		} catch (Exception e) {
			LOGGER.error("H5GameBoutiqueCategoryDaoImpl.delGameBoutiqueCategory failed, e : " + e);
		}
	}

	@Override
	// 更新游戏游戏分类
	public void updateGameBoutiqueCategory(String id, String name, String logo,
			String order, String modifier) {
		try {
	...
	public Map<String, Object> getAllGame(int page, String cid, String name,
		...
		try {
			StringBuffer sql = new StringBuffer("SELECT COUNT(1)" +
					...
					" AND NOT EXISTS" +
					" (SELECT C.C_GID FROM t_game_h5_boutique_category C WHERE" +
			...
			LOGGER.error("H5GameBoutiqueCategoryDaoImpl.getAllGame failed, e : " + e);
		}
		return ret;
	}
	...
	public Map<String, Object> getCurrCateGame(int page, String cid, String name) {
		...
		try {
			StringBuffer sql = new StringBuffer("SELECT COUNT(1)" +
					" FROM t_game_h5_boutique_category T,T_GAME_H5_INFO G WHERE " +
			...
			LOGGER.error("H5GameBoutiqueCategoryDaoImpl.getCurrCateGift failed, e : " + e);
		}
		return ret;
	}
	...
	public int isExist(String cid, String gid) {
		try {
			String sql = "SELECT COUNT(1) FROM t_game_h5_boutique_category " +
					" WHERE C_CID = ? AND C_GID = ?";
			return jdbcTemplate.queryForObject(sql, new Object[]{cid, gid}, Integer.class);
		} catch (Exception e) {
			LOGGER.error("H5GameBoutiqueCategoryDaoImpl.isExist failed, e : " + e);
		}
		return 0;
	}
	...
	public void update(String cid, String gid, String order) {
		try {
			String sql = "UPDATE t_game_h5_boutique_category SET" +
					" C_ORDER = ? WHERE C_CID = ? AND C_GID = ?";
			jdbcTemplate.update(sql, new Object[]{order,cid,gid});
		} catch (Exception e) {
			LOGGER.error("H5GameBoutiqueCategoryDaoImpl.update failed, e : " + e);
		}
	}
	...
	public void remove(String cid, String gid) {
		try {
			String sql = "DELETE FROM t_game_h5_boutique_category" +
					" WHERE C_CID = ? AND C_GID = ?";
			jdbcTemplate.update(sql, new Object[]{cid,gid});
		} catch (Exception e) {
			LOGGER.error("H5GameBoutiqueCategoryDaoImpl.remove failed, e : " + e);
		}
	}
