...
public class EKFreeCallDaoImpl implements EKFreeCallDao {
    ...
    public int selectAll() {
        ...
        int count = 0;
        Map<String, Object> ret = new HashMap<String, Object>();
        String sql = "SELECT T.C_ID,T.C_APPACCOUNTID,T.C_FYACCOUNTID,T.C_CREATEDATE,CASE WHEN T.C_STATUS = '1' THEN '启用' ELSE '禁用' END AS STATUS,T.C_GLOBALMOBILEPHONE,T.C_FYACCOUNTPWD,T.C_STATUS FROM T_FY_USER T WHERE 1=1";
        try {
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                String C_APPACCOUNTID = StrUtils.emptyOrString(map.get("C_APPACCOUNTID"));
                String C_GLOBALMOBILEPHONE = StrUtils.emptyOrString(map.get("C_GLOBALMOBILEPHONE"));
                String C_FYACCOUNTPWD = StrUtils.emptyOrString(map.get("C_FYACCOUNTPWD"));
                ...
                if (!"0".equals(resultCode)) {
                    try {
                        ...
                        //System.out.println(C_APPACCOUNTID+"==="+C_FYACCOUNTID+"==="+C_FYACCOUNTPWD+"==="+C_GLOBALMOBILEPHONE);
                        count += 1;
                    } catch (Exception e1) {
    ...
    @Override
    // 获取所有飞语账号
    public List<Map<String, Object>> getAllFyAccout() {
        String sql = "SELECT T.C_FYACCOUNTID, T.C_APPACCOUNTID, T.C_GLOBALMOBILEPHONE" +
                " FROM T_FY_USER T WHERE T.C_STATUS = 1";
        return jdbcTemplate.queryForList(sql);
    }

    @Override
    // 更新已存在的异常账号信息
    public int changeFyAccount(String fyAccId, Object fyAccountId, Object fyAccountPwd, Object addDate) {
        String sql = "UPDATE T_FY_USER T SET T.C_FYACCOUNTID = ?, T.C_FYACCOUNTPWD = ?," +
                " T.C_CREATEDATE = ? WHERE T.C_FYACCOUNTID = ?";
        return jdbcTemplate.update(sql, new Object[]{fyAccountId, fyAccountPwd, addDate, fyAccId});
    }

    @Override
    // 修改通话时间表中的飞语账号
    public int changeFyAccountInCallingTime(String fyAccId, Object fyAccountId) {
        String sql = "UPDATE T_FY_USER_TIME T SET T.C_FYACCOUNTID = ? WHERE T.C_FYACCOUNTID = ?";
        return jdbcTemplate.update(sql, new Object[]{fyAccountId, fyAccId});
    }
