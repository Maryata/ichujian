 ...
 * 游戏帮：H5游戏推荐
 */
public class H5GameHotSearchDaoImpl implements H5GameHotSearchDao {

	private static final Logger LOGGER = Logger
			.getLogger(H5GameHotSearchDaoImpl.class);
	...
	public Map<String, Object> getAllGame(int page, String name, String gameCate) {
		...
		try {
			...
			LOGGER.error("H5GameHotSearchDaoImpl.getAllGame failed, e : "
					+ e);
		}
		return ret;
	}
	...
	public Map<String, Object> recommendGameList(int page, String name) {
		...
		try {
			StringBuffer sql = new StringBuffer("SELECT COUNT(1)" +
					" FROM T_GAME_H5_INFO T, T_GAME_POPULAR_SEARCH R WHERE" +
			...
			LOGGER.error("H5GameHotSearchDaoImpl.recommendList failed, e : " + e);
		}
		return ret;
	}
	...
	public int isExist(String gid) {
		try {
			String sql = "SELECT COUNT(1) FROM T_GAME_POPULAR_SEARCH WHERE C_GID = ? AND C_TYPE = 2";
			return jdbcTemplate.queryForObject(sql, new Object[]{gid}, Integer.class);
		} catch (Exception e) {
			LOGGER.error("H5GameHotSearchDaoImpl.isExist failed, e : " + e);
		}
		return 0;
	}
	...
	public void add(String gid, String order) {
		try {
			String sql = "INSERT INTO T_GAME_POPULAR_SEARCH" +
					...
					" (seq_game_popular_search.NEXTVAL,?,2,?)";
			jdbcTemplate.update(sql, new Object[]{gid,order});
		} catch (Exception e) {
			LOGGER.error("H5GameHotSearchDaoImpl.add failed, e : " + e);
		}
	}
	...
	public void update(String gid, String order) {
		try {
			...
			LOGGER.error("H5GameHotSearchDaoImpl.update failed, e : " + e);
		}
	}
	...
	public void remove(String gid) {
		try {
			String sql = "DELETE FROM T_GAME_POPULAR_SEARCH WHERE C_GID = ? AND C_TYPE = 2";
			jdbcTemplate.update(sql, new Object[]{gid});
		} catch (Exception e) {
			LOGGER.error("H5GameHotSearchDaoImpl.remove failed, e : " + e);
		}
	}
