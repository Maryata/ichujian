 ...
 */
@Service
public class EKH5GameCategoryServiceImpl implements EKH5GameCategoryService {
	
	@Autowired
	private EKH5GameCategoryDao eKH5GameCategoryDao;

	@Override
	// 获取游戏分类（下拉菜单）
	public List<Map<String, Object>> getGameCategoryList() {
		return eKH5GameCategoryDao.getGameCategoryList();
	}

	@Override
	// 分页显示游戏分类
	public Map<String, Object> gameCategoryList(int page) {
		return eKH5GameCategoryDao.gameGiftCateList(page);
	}
	...
	public void addGameCate(String id, String name, String logo, String order,
			String modifier) {
		eKH5GameCategoryDao.addGameGiftCate(id, name, logo, order, modifier);
	}

	@Override
	// 删除分类
	public void delGameCate(String id) {
		eKH5GameCategoryDao.delGameGiftCate(id);
	}
	...
	public void updateGameCate(String id, String name, String logo,
		...
		eKH5GameCategoryDao.updateGameGiftCate(id, name, logo, order, modifier);
	}
	...
	public Map<String, Object> getAllGame(int page, String cid, String name,
			String gameCate) {
		return eKH5GameCategoryDao.getAllGame(page, cid, name, gameCate);
	}

	@Override
	// 查询当前游戏分类的游戏
	public Map<String, Object> getCurrCateGame(int page, String cid, String name) {
		return eKH5GameCategoryDao.getCurrCateGame(page, cid, name);
	}
	...
	public void handleCate(String cid, String gid, String removeGid,
		...
		if(null!=gids && gids.length>0){
			for (int i = 0; i < gids.length; i++) {
				...
				// 查询礼包是否已经存在
				int cateCount = eKH5GameCategoryDao.isExist(cid, gids[i]);
				if(cateCount==0){
					// 如果不存在，新增
					eKH5GameCategoryDao.add(cid, gids[i], orders[i]);
				}else{
					// 如果存在，更新排序
					eKH5GameCategoryDao.update(cid, gids[i], orders[i]);
				}
			}
		}
		// 移除出分类
		if(null!=removeGids && removeGids.length>0){
			for (String id : removeGids) {
				...
				eKH5GameCategoryDao.remove(cid, id);
			}
		}
	}
