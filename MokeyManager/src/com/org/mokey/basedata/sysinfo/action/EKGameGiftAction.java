...
import com.org.mokey.basedata.sysinfo.service.*;
...
public class EKGameGiftAction extends AbstractAction {
    ...
    public String toGiftList() {
        log.info("into EKGameGiftAction.toActList");
        try {
            // 查询所有游戏
            ...
            log.error("EKGameGiftAction.toActList failed, e : " + e);
        }
        return "gameGiftList";
    }
    ...
    public String gameGiftList() {
        ...
        try {
            ...
            List<Map<String, Object>> list = eKGameGiftService.gameGiftList(gid, name, page, gName);
    ...
    public String getTotal() {
        ...
        String gName = getParameter("gName");// 游戏名
        String name = getParameter("name");// 礼包名
        log.info("into EKGameGiftAction.getTotalCol");
        log.info("gid = " + gid + ", name = " + name);
        try {
            Integer rows = 10;// 每页10条
            Integer totalPage = 1;
            // 总条数
            Integer total = eKGameGiftService.getTotal(gid, name, gName);
            // 计算总页数
            ...
            log.error("EKGameGiftAction.getTotal failed,", e);
        }
        return NONE;
    }
    ...
    public String deleteGameGift() {
        ...
        log.info("into EKGameGiftAction.GameGiftAction");
        log.info("id = " + id);
        try {
            // 删除
            eKGameGiftService.deleteGameGift(id);
        } catch (Exception e) {
            log.error("EKGameGiftAction.getTotal failed,", e);
        }
        return NONE;
    ...
    public String toGameGiftAdd() {
        log.info("into EKGameGiftAction.toGameGiftAdd");
        try {
            // 查询所有游戏
            List<Map<String, Object>> games = appInfoService.getGameAppInfoList();
            getRequest().setAttribute("games", games);
            // 查询所有游戏礼包分类
            List<Map<String, Object>> cate = eKGameGiftCateService.getGiftCateList();
            getRequest().setAttribute("cate", cate);
            // 主键id
            String newId = JdbcTemplateUtils.getSeqId(jdbcTemplate, "SEQ_EK_GAME_ACTIVITY_INFO");
            getRequest().setAttribute("newId", newId);
            // 添加/编辑的标识
            getRequest().setAttribute("addFlag", 1);
        } catch (Exception e) {
            log.error("EKGameGiftAction.toGameGiftAdd failed, e : " + e);
        }
    ...
    public String toGameGiftEdit() {
        ...
        log.info("into EKGameGiftAction.toGameGiftEdit");
        ...
        try {
            ...
            getRequest().setAttribute("cate", cate);
            // 根据id查询礼包
            List<Map<String, Object>> list = eKGameGiftService.queryGiftById(editId);
            String editGid = "";
            String editOrder = "";
            String editCount = "";
            String editName = "";
            ...
            log.error("EKGameGiftAction.toGameGiftEdit failed, e : " + e);
        }
        return "toGameGiftAdd";
    }
    ...
    public String saveGameGift() {
        ...
        log.info("into EKGameGiftAction.saveGameGift");
        log.info("id = " + id + ", gid = " + gid + ", name = " + name + ", cate = " + cate +
                ", depict = " + depict + ", sdate = " + sdate + ", edate = " + edate +
                ", method = " + method + ", count = " + count + ", order = " + order);
        try {
            eKGameGiftService.saveGameGift(id, gid, name, depict, sdate, edate, method, count, order, cate);
        } catch (Exception e) {
            log.error("EKGameGiftAction.saveGameGift failed, e : " + e);
        }
        return "saveGameGift";
    }
    ...
    public String upload() {
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        log.error("EKGameGiftAction.upload failed, e : " + e);
            ...
            eKGameGiftService.upload(list, id);
        } catch (Exception e) {
            log.error("EKGameGiftAction.upload failed, e : " + e);
    ...
    public String toGiftCodesList() {
        ...
        log.info("into EKGameGiftAction.toGiftCodes");
        log.info("gid = " + gid);
        try {
            getRequest().setAttribute("gid", gid);
        } catch (Exception e) {
            log.error("EKGameGiftAction.toGiftCodes failed,", e);
        }
    ...
    public String getCodesListByGid() {
        ...
        log.info("into EKGameGiftAction.getCodesListByGid");
        log.info("gid = " + gid + ", state = " + state +
                ", sDate = " + sDate + ", eDate = " + eDate);
        try {
            // 分页查询
            retmap = eKGameGiftService.getCodesListByGid(page, gid, state, sDate, eDate);
            retmap.put("status", "Y");
            // 回写查询结果
            this.writeJSONToResponse(retmap);
        } catch (Exception e) {
            retmap.put("status", "N");
            log.error("EKGameGiftAction.getCodesListByGid failed,", e);
        }
        return NONE;
    }
    ...
    public String delCode() {
        ...
        log.info("into EKGameGiftAction.delCode");
        log.info("gid = " + gid + ", id = " + id);
        try {
            // 分页查询
            eKGameGiftService.delCode(gid, id);
        } catch (Exception e) {
            log.error("EKGameGiftAction.delCode failed,", e);
        }
        return NONE;
    }
    ...
    public String batchDel() {
        String gid = getParameter("gid");// 礼包id
        String id = getParameter("id");// 礼包码id
        log.info("into EKGameGiftAction.batchDel");
        log.info("gid = " + gid + ", id = " + id);
        try {
            // 分页查询
            eKGameGiftService.batchDel(gid, id);
        } catch (Exception e) {
            log.error("EKGameGiftAction.batchDel failed,", e);
        }
        return NONE;
    }
    ...
    public String exportExcel() {
        ...
        log.info("into EKGameGiftAction.exportExcel");
        log.info("gid = " + gid + ", state = " + state);
        try {
            // 按“领取状态”查询所有指定礼包的礼包码
            List<Map<String, Object>> list = eKGameGiftService.getCodesByGidNState(gid, state);
            ...
            log.error("EKGameGiftAction.exportExcel failed,", e);
        }
        return NONE;
    }
}
