...
public class EKFreeCallAction extends AbstractAction {
    ...
    public String addAccount() {
        ...
        if ("1".equals(type)) {
            ...
            if (StrUtils.isNotEmpty(maplist)) {
                ...
                for (String str : set) {// 遍历set去出里面的的Key
                    ...
                    addAcount_common(regid, phone, id, FYiD, type);
                }
            }
        } else if ("2".equals(type)) {
            ...
            for (String str : set) {// 遍历set去出里面的的Key
                //System.out.println(maplist.get(str) + " ==" + str);// 通过key,取得value打印出来
                String phone = maplist.get(str);
                String regid = str;
                addAcount_common(regid, phone, null, null, type);
            }
        }
     ...
     * @return
     */
    public void addAcount_common(String regid, String phone, String id, String FYiD, String type) {
        /****添加飞语账号****/
        paramMap = new HashMap<String, String>();
        Long time = freeCall.GetTime();
        paramMap.put("appId", freeCall.GetAppId());//appId
        ...
        if ("0".equals(resultCode)) {
            ...
            if ("0".equals(resultCode1)) {
                ...
                if ("1".equals(type)) {//更新
                    ...
                    int addAccount = eKFreeCallService.insertAccount(regid, fyAccountId, fyAccountPwd, addDate, status, phone);
                    if (addAccount > 0) {
                        eKFreeCallService.addTime(fyAccountId);
                    }
                }
    ...
    public String registerList() {
        Map<String, Object> retmap = new HashMap<String, Object>();
        log.info("into EKFreeCallAction.registerList");
        try {
            retmap = eKFreeCallService.registerList();
            // 回写查询结果
            this.writeJSONToResponse(retmap);
        } catch (Exception e) {
            retmap.put("status", "N");
            log.error("EKFreeCallAction.registerList failed, e : " + e);
        }
        return NONE;
    ...
    public String deleteFy() {
        ...
        if ("1".equals(status)) {
            ...
            Long time = freeCall.GetTime();
            paramMap.put("appId", freeCall.GetAppId());//appId
            paramMap.put("fyAccountId", fyId);//账号id
            paramMap.put("ti", String.valueOf(time));//时间
            paramMap.put("ver", freeCall.GetVer());//版本
            paramMap.put("au", freeCall.getAuValue(time));//AU
            data = HttpUtil.sendPost(e.freecall_disable, paramMap, "UTF-8");
            ...
            if ("0".equals(resultCode)) {
                //删除时间 和 用户
                int deleteTime = eKFreeCallService.deleteTime(fyId);
                int deleteUser=eKFreeCallService.deleteUser(fyId);
                map.put("resultMsg","禁用并删除飞语成功！");
            }else{
                //resultMsg
                map.put("resultMsg","禁用并删除飞语失败！");
            }
        } else {
            //删除时间 和 用户
            int deleteTime = eKFreeCallService.deleteTime(fyId);
            int deleteUser=eKFreeCallService.deleteUser(fyId);
            map.put("resultMsg","删除飞语成功！");
        }
        try {
            this.writeJSONToResponse(map);
        } catch (IOException e1) {
